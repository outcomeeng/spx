...
| Target vocabulary             | The six kinds, ordered kind decision procedure, containment rules, openings, operational terms, tier/state vocabulary, and deprecated or ambiguous terms that must not drive placement. |
...
| Operational concern placement | Persistence, delivery, backend, node state, tier, and status-claim concerns assigned to the node that owns their semantics or contract.                                                 |
