...
Read `${CLAUDE_SKILL_DIR}/references/projection-views.md`, `${CLAUDE_SKILL_DIR}/references/target-vocabulary.md`, and `${CLAUDE_SKILL_DIR}/references/ordering-evidence.md` before deriving a slice.
...
5. If reviewed views do not exist, run `${CLAUDE_SKILL_DIR}/workflows/project-target-structure.md` or `${CLAUDE_SKILL_DIR}/workflows/split-fused-node.md`, then run `${CLAUDE_SKILL_DIR}/workflows/audit-projection.md` before deriving the slice.
6. Check view freshness against the current invocation scope, authority, inventory, kind decision, operational placement, receiver, containment, dependency evidence, context visibility, ordering evidence, active migration, parked scope, contradiction, and unresolved-decision views. For `spx/` scope, also check product top-level mapping freshness. If any upstream view changed, rerun the projection or split workflow and then rerun `${CLAUDE_SKILL_DIR}/workflows/audit-projection.md` before deriving the slice.
7. Select one slice whose receiver is known.
8. Check whether SPX can represent the slice today. Name required support for suffix admission, authoring, validation, context loading, status projection, status claims, tier, test/eval/audit discovery, rendering, and refactor tooling.
...
| Support gate       | Required SPX support table covers suffix admission, authoring, validation, context loading, status projection, status claims, tier, evidence discovery, rendering, and refactor tooling.                                                                                                              | A relevant support area is omitted or represented as present without evidence.          |
...
- [ ] Suffix admission, context loading, tier, and status-claim impacts are named when relevant.
