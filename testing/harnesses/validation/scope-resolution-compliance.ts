...
function createDependencyGraphResult(): Awaited<
  ReturnType<CircularDependencyGraphRunner>
> {
  return {
    output: {
      modules: [],
...
function expectTemporaryConfigPathInsideNodeModules(
  productDir: string,
  ...
  expect(
    configPath?.startsWith(
      join(productDir, ...TEMPORARY_TSCONFIG_PARENT_SEGMENTS),
    ),
  ).toBe(true);
}
...
const nestedSourceFile = join(
  narrowSourceDirectory,
  VALIDATION_PIPELINE_DATA.cleanSourceFileName,
);
const deepSourceFile = join(
  deepSourceDirectory,
  VALIDATION_PIPELINE_DATA.cleanSourceFileName,
);
...
class ErrorThenCloseRunner implements ProcessRunner {
  ...
  spawn(
    ...
    queueMicrotask(() => {
      child.emit(
        VALIDATION_SUBPROCESS_EVENTS.ERROR,
        new Error(this.errorMessage),
      );
      child.emit(
        VALIDATION_SUBPROCESS_EVENTS.CLOSE,
...
describe("ALWAYS: TypeScript scope resolution uses the requested project root", () => {
  it("discovers TypeScript directories under the requested project root", async () => {
    await withTestEnv({}, async (env) => {
      ...
      expect(scope.directories).toContain(
        VALIDATION_PIPELINE_DATA.scopeResolutionDirectoryName,
      );
    });
  });
  ...
  it("resolves array-based TypeScript config extends without crashing", async () => {
    await withTestEnv({}, async (env) => {
      ...
      await env.writeRaw(
        ...
        JSON.stringify({
          extends: ["./base-includes.json", "./base-excludes.json"],
        }),
      );

      const scope = getTypeScriptScope(
  ...
  it("ignores non-TypeScript include patterns when resolving TypeScript scope", async () => {
    await withTestEnv({}, async (env) => {
      ...
      await env.writeRaw(
        ...
        JSON.stringify({
          include: [
            VALIDATION_PIPELINE_DATA.recursiveMarkdownSourceFilePattern,
          ],
        }),
      );
  ...
  it("classifies TypeScript source include patterns without widening extensionless files", () => {
    ...
    expect(
      typeScriptScopePatternTargetsTypeScriptSource(topLevelSourceFile),
    ).toBe(true);
    expect(
      typeScriptScopePatternTargetsTypeScriptSource(extensionlessNestedPath),
    ).toBe(false);
    expect(
      typeScriptScopePatternTargetsTypeScriptSource(
        `**/${VALIDATION_PIPELINE_DATA.extensionlessSourceFileName}`,
  ...
  it("keeps literal nested directory includes scoped to their TypeScript subtree", async () => {
    await withTestEnv({}, async (env) => {
      ...
      await env.writeRaw(
        TSCONFIG_FILES.full,
        JSON.stringify({ include: [narrowSourceDirectory] }),
      );

      const scope = getTypeScriptScope(VALIDATION_SCOPES.FULL, env.productDir);

      expect(scope.filePatterns).toEqual([
        `${narrowSourceDirectory}${TYPESCRIPT_SCOPE_DIRECTORY_PATTERN_SUFFIX}`,
      ]);
  ...
  it("keeps literal directory includes whose final segment has a TypeScript suffix", async () => {
    await withTestEnv({}, async (env) => {
      ...
      await env.writeRaw(
        TSCONFIG_FILES.full,
        JSON.stringify({
          include: [VALIDATION_PIPELINE_DATA.modernSourceFileName],
        }),
      );
      ...
      expect(
        pathPassesTypeScriptScope(modernNamedDirectorySourceFile, scope),
      ).toBe(true);
    });
  });
  ...
  it("does not widen scope for dotless literal file includes", async () => {
    await withTestEnv({}, async (env) => {
      ...
      await env.writeRaw(
        TSCONFIG_FILES.full,
        JSON.stringify({ include: [extensionlessNestedPath] }),
      );
  ...
  it.each(VALIDATION_PIPELINE_DATA.extensionSpecificExcludeScenarios)(
    ...
    async ({ excludePattern, sourceFileName }) => {
      ...
      await withTestEnv({}, async (env) => {
        ...
        await env.writeRaw(
          TSCONFIG_FILES.full,
          JSON.stringify({ exclude: [excludePattern] }),
        );
        ...
        expect(scope.directories).not.toContain(
          VALIDATION_PIPELINE_DATA.sourceDirectoryName,
        );
        expect(pathPassesTypeScriptScope(excludedSourceFile, scope)).toBe(
          false,
        );
      });
    },
  ...
  it("keeps directories that contain TypeScript sources not matched by an extension-specific exclude", async () => {
    await withTestEnv({}, async (env) => {
      ...
      await env.writeRaw(
        TSCONFIG_FILES.full,
        JSON.stringify({
          exclude: [VALIDATION_PIPELINE_DATA.typeScriptJsxSourceFilePattern],
        }),
      );
    ...
    await withTestEnv({}, async (env) => {
      ...
      await env.writeRaw(
        join(
          ...
          basename(sampleLiteralTestValue(LITERAL_TEST_GENERATOR.sourceFilePath())),
        ),
        "",
      );
      await env.writeRaw(
  ...
  it("runs TypeScript validation from the requested project root", async () => {
    await withTestEnv({}, async (env) => {
      ...
      expect(
        checkedPaths.every((path) => path.startsWith(env.productDir)),
      ).toBe(true);
      expect(
        runner.commands.every((command) => command.startsWith(env.productDir)),
      ).toBe(true);
      expect(
        runner.options.every((options) => options.cwd === env.productDir),
      ).toBe(true);
    });
  });
  ...
  it("preserves TypeScript root directory operands through validation include paths", async () => {
    await withValidationEnv(
      { fixture: PROJECT_FIXTURES.CLEAN_PROJECT },
      async ({ path }) => {
        const secondarySourceDirectory = join(
          path,
  ...
  it("intersects TypeScript directory operands with validation include paths", async () => {
    await withValidationEnv(
      ...
      async ({ path }) => {
        ...
        expect(scopeConfig.filteredByValidationPathNoMatches).not.toBe(true);
        expect(
          scopeConfig.filePatterns.length + scopeConfig.directories.length,
        ).toBeGreaterThan(0);
        expect(scopeConfig.filePatterns.join("\n")).not.toContain(
          VALIDATION_PIPELINE_DATA.secondarySourceDirectoryName,
        );
        expect(scopeConfig.directories).not.toContain(
          VALIDATION_PIPELINE_DATA.secondarySourceDirectoryName,
        );
      },
    );
      ...
      async ({ path }) => {
        ...
        await writeFile(
          join(
            ...
            VALIDATION_PIPELINE_DATA.excludedSourceFileName,
          ),
          VALIDATION_PIPELINE_DATA.secondaryTypeErrorSourceContent,
        );
        await writeFile(
  ...
  it("preserves validation include directories below TypeScript directory operands", async () => {
    await withValidationEnv(
      ...
      async ({ path }) => {
        ...
        await writeFile(
          join(
            narrowedSourceDirectory,
            VALIDATION_PIPELINE_DATA.narrowedSourceFileName,
          ),
          VALIDATION_PIPELINE_DATA.secondaryTypeErrorSourceContent,
        );
        await writeFile(
    ...
    await withTestEnv(
      ...
      async (env) => {
        ...
        expect(result.output).toBe(
          TYPESCRIPT_VALIDATION_MESSAGES.NO_VALIDATION_PATH_TARGETS,
        );
      },
    );
  });
  ...
  it("runs config-filtered Knip validation through a scoped temporary config", async () => {
    await withTestEnv({}, async (env) => {
      ...
      expect(JSON.parse(writtenConfigs[0] ?? "{}")).toEqual({
        extends: join(env.productDir, TSCONFIG_FILES.full),
        include: [
          ...expectedKnipDirectorySourcePatterns(
            VALIDATION_PIPELINE_DATA.sourceDirectoryName,
          ).map((pattern) => join(env.productDir, pattern)),
          join(
            env.productDir,
            VALIDATION_PIPELINE_DATA.productionScopeFilePattern,
          ),
  ...
  it("cleans config-filtered Knip temporary config once when error and close both fire", async () => {
    const errorMessage = sampleLiteralTestValue(
  ...
  it("runs circular validation with project-root-anchored dependency-cruiser inputs", async () => {
    await withTestEnv({}, async (env) => {
      ...
      await env.writeRaw(
        TSCONFIG_FILES.full,
        JSON.stringify({
          include: [VALIDATION_PIPELINE_DATA.scopeResolutionSourceFile],
        }),
      );
      await env.writeRaw(
        VALIDATION_PIPELINE_DATA.scopeResolutionSourceFile,
      ...
      const deps: CircularDeps = {
        ...
        [CIRCULAR_DEPS_KEYS.EXTRACT_TYPESCRIPT_CONFIG]: (tsConfigFile) => {
          ...
          return defaultCircularDeps[
            CIRCULAR_DEPS_KEYS.EXTRACT_TYPESCRIPT_CONFIG
          ](tsConfigFile);
        },
      };
      ...
      expect(paths).toEqual(
        expectedDependencyCruiserSourcePatterns(
          VALIDATION_PIPELINE_DATA.scopeResolutionDirectoryName,
        ),
      );
      expect(config?.baseDir).toBe(env.productDir);
      expect(config?.tsConfig?.fileName).toBe(
        join(env.productDir, TSCONFIG_FILES.full),
      );
      expect(config?.tsPreCompilationDeps).toBe(
        DEPENDENCY_CRUISER_TS_PRE_COMPILATION_DEPS,
      );
      expect(config?.moduleSystems).toEqual([
        ...DEPENDENCY_CRUISER_MODULE_SYSTEMS,
      ]);
      expect(config?.includeOnly).toEqual({
        path: DEPENDENCY_CRUISER_TYPESCRIPT_SOURCE_PATTERN,
      });
      const dependencyCruiserSourceMatcher = new RegExp(
        DEPENDENCY_CRUISER_TYPESCRIPT_SOURCE_PATTERN,
        "u",
      );
      expect(
        dependencyCruiserSourceMatcher.test(
          VALIDATION_PIPELINE_DATA.cleanSourceFileName,
        ),
      ).toBe(true);
      expect(
      ...
      expect(
        dependencyCruiserSourceMatcher.test(
          VALIDATION_PIPELINE_DATA.commonjsSourceFileName,
        ),
      ).toBe(true);
      expect(
        dependencyCruiserSourceMatcher.test(
      ...
      expect(
        dependencyCruiserSourceMatcher.test(
          VALIDATION_PIPELINE_DATA.modernDeclarationSourceFileName,
        ),
      ).toBe(false);
      expect(
        dependencyCruiserSourceMatcher.test(
          VALIDATION_PIPELINE_DATA.commonjsDeclarationSourceFileName,
        ),
      ).toBe(false);
...
describe("ALWAYS: the temporary tsconfig reproduces the project's TypeScript resolution", () => {
  it("writes the scope-filtered temporary config inside the project's node_modules and fabricates no compiler options", async () => {
    await withTestEnv({}, async (env) => {
      ...
      const writtenConfig = JSON.parse(writtenConfigs[0] ?? "{}");
      expect(writtenConfig.extends).toBe(
        join(env.productDir, TSCONFIG_FILES.full),
      );
      expect(writtenConfig.compilerOptions).toEqual({ noEmit: true });
    });
