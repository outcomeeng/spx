...
function buildSpecTreeConfig(
  kind: string,
  definition: ConfigKindDefinition,
): Config {
  return {
...
function readFirstKind(
  category: SpecTreeKindCategory,
): readonly [string, ConfigKindDefinition] {
  const entry = Object.entries(KIND_REGISTRY).find(
    ([, definition]) => definition.category === category,
  );
  if (entry === undefined) {
    throw new Error(
      `Missing registered ${category} kind for resolution-scope fixture`,
    );
  }
  return entry;
}

function readResolvedSpecTree(
  config: Config,
): typeof specTreeConfigDescriptor.defaults {
  const validated = specTreeConfigDescriptor.validate(
    config[specTreeConfigDescriptor.section],
  );
...
export function registerConfigResolutionScopeCompliance(): void {
  describe("resolveConfig — resolution scope (C1)", () => {
    it("reads only the config file at the supplied productDir for every config-owned format", async () => {
      const scope = sampleConfigTestValue(
      ...
      const [rootKind, rootDefinition] = readFirstKind(
        SPEC_TREE_KIND_CATEGORY.NODE,
      );
      const rootConfig = buildSpecTreeConfig(rootKind, rootDefinition);
      const parentConfig = buildSpecTreeConfig(
        parentOnly.kind,
        parentOnly.definition,
      );
      const nestedConfig = buildSpecTreeConfig(
        nestedOnly.kind,
        nestedOnly.definition,
      );
      ...
      for (const format of CONFIG_FILE_FORMAT_ORDER) {
        await withTestEnv(parentConfig, async ({ productDir, writeRaw }) => {
          ...
          await writeRaw(
            writeConfigPath(
              join(scope.productDirectory, scope.nestedDirectory),
              format,
            ),
          ...
          const result = await resolveConfig(
            join(productDir, scope.productDirectory),
            [specTreeConfigDescriptor],
          );
          ...
          if (result.ok) {
            ...
            expect(Object.keys(specTree.kinds)).toEqual(
    ...
    it("exposes config-owned read, parse, and serialize APIs for every declared config format", async () => {
      const [kind, definition] = readFirstKind(
        SPEC_TREE_KIND_CATEGORY.DECISION,
      );
        ...
        await withTestEnv({}, async ({ productDir, writeRaw }) => {
          for (const registeredFormat of CONFIG_FILE_FORMAT_ORDER) {
            await rm(
              join(
                productDir,
                CONFIG_FILE_DEFINITIONS[registeredFormat].filename,
              ),
              { force: true },
            );
          }
          await writeRaw(
            CONFIG_FILE_DEFINITIONS[format].filename,
            serializeConfig(format, config),
          );
          ...
          expect(parsed.ok).toBe(true);
          if (!parsed.ok) return;
          expect(readResolvedSpecTree(parsed.value)).toEqual(
            readResolvedSpecTree(config),
          );

          const serialized = serializeConfigFileSections(
            read.value.file.format,
            parsed.value,
          );
          expect(serialized.ok).toBe(true);
          if (!serialized.ok) return;
          expect(parseSerialized(read.value.file, serialized.value)).toEqual(
            parsed.value,
          );
        });
      }
