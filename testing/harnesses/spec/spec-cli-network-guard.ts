...
function rejectNetworkAttempt(label: string): never {
...
function networkGuard(label: string): () => never {
  return () => rejectNetworkAttempt(label);
}

function installNetworkGuard(target: object, namespace: string, methodNames: readonly string[]): void {
