...
function registerWorktreeCommands(worktreeCmd: Command): void {
  worktreeCmd
    ...
    .action(async (options: { sessionId: string; worktreesDir?: string }) => {
      const result = await claimCommand({ sessionId: options.sessionId, worktreesDir: options.worktreesDir });
  ...
  worktreeCmd
    ...
    .option(`${WORKTREE_CLI.WORKTREES_DIR_FLAG} <path>`, "Explicit .spx/worktrees directory")
    .action(async (worktree: string, options: { format?: string; worktreesDir?: string }) => {
      const result = await statusCommand({ worktree, format: options.format, worktreesDir: options.worktreesDir });
      if (!result.ok) handleError(result.error);
  ...
  worktreeCmd
    ...
    .action(async (options: { worktreesDir?: string }) => {
      const result = await releaseCommand({ worktreesDir: options.worktreesDir });
      if (!result.ok) handleError(result.error);
    });
}
