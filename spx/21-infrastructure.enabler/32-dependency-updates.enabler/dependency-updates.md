...
- ALWAYS: an update branch is rebased onto its base branch only when it conflicts, so merging one update leaves every other open update's continuous-integration results intact and eligible to merge in the same window ([audit])
