...
PROVIDES a repository-root Renovate configuration that raises pull requests for outdated and vulnerable dependencies — routine updates held until a minimum release age and throttled by per-hour and concurrency limits, vulnerability fixes and the product's own reusable-workflow updates at any time
...
- ALWAYS: routine updates across every manager are withheld until a release reaches a minimum age, while vulnerability fixes and updates to the product's own reusable workflows bypass the wait ([audit])
