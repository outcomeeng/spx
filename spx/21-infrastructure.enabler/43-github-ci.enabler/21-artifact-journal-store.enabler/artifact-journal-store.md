...
- ALWAYS: a run becomes sealed only after its retention upload succeeds, so a retention failure leaves the run unsealed and a later seal retries rather than stranding a sealed-but-unretained run ([test](tests/artifact-journal-store.compliance.l1.test.ts))
