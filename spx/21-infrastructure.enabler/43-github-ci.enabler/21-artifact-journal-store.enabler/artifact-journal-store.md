...
- ALWAYS: hydration skips a restored artifact directory that does not hold its run file, so a malformed or empty restored artifact does not fail the opening run ([test](tests/artifact-journal-store.compliance.l1.test.ts))
