...
- ALWAYS: product instructions identify the main checkout through `spx/15-worktree-management.pdr.md`, preserve its default-branch occupancy, refresh the operator-visible CLI only through the `rebuild-dist` hook's install and build on `git pull` there after every merge with the hook outcome and resulting executable verified — the one-time bootstrap of a checkout that carries no hook or no `dist/` being the sole explicit install and build admitted — and create a release tag only after that verification ([audit])
...
- NEVER: product instructions detach the main checkout, switch it away from the default branch, or direct a linked worktree to check out the default branch ([audit])
