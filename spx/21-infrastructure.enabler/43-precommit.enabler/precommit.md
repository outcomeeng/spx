...
PROVIDES lefthook-managed local hook machinery: a selective changed-set runner for product config files and default-config staged changes, a non-default-config operand runner for config-classified source and test files, a fixture-exclusion drift check, a main-checkout-gated dist rebuild path for pull and rebase events, and a post-checkout dependency-install gate for checkout events
...
- Given non-default precommit config with custom source directories or test pattern, when the runner executes for retained staged source or test files, then the operand runner receives the config-classified source or test paths and the runner's exit code is observed directly ([test](tests/run.scenario.l1.test.ts))
...
- Non-default-config operand invocation shape: retained source files map to `["related", "--run", ...sourceFiles, ...testFiles]`, and retained test files with no source files map to `["--run", ...testFiles]` ([test](tests/build-args.mapping.l1.test.ts))
- Non-default-config product config invocation shape: any retained product config file maps to `["test", "--changed", "--staged", "--base", "HEAD"]` ([test](tests/build-args.mapping.l1.test.ts))
...
- ALWAYS: staged product config files route through changed-set planning under every precommit config, while non-default precommit `sourceDirs` or `testPattern` route config-classified source and test paths through the operand runner so precommit-owned classification remains honored when it differs from the product-owned changed-set roots and TypeScript test suffixes ([test](tests/run.scenario.l1.test.ts))
