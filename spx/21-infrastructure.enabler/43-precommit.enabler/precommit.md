...
CAN keep the main checkout's packaged `dist/` current after incoming changes, preserve its default-branch occupancy while product instructions refresh the operator-visible CLI through that pull alone, skip rebuilds in non-main worktrees, install dependencies in any worktree advanced to a new commit when the checkout changes the lockfile, and leave build, validation, test execution, and SonarQube fixture-exclusion synchronization to CI, cloud analysis, or explicit operator and agent commands
...
- ALWAYS: product instructions keep the default branch checked out in the main checkout, refresh the operator-visible CLI only through the `rebuild-dist` hook's install and build on `git pull` there after every merge — verified by the hook outcome and the resulting executable, with no release-tag condition — and create a release tag only after that shared-CLI verification, so Git refuses the occupied default branch to linked worktrees and the shared executable tracks the merged default branch ([audit])
