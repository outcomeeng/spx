...
- ALWAYS: the mirror runs each rule at one of two enforcement tiers — an error tier whose severity blocks `spx validation` on any finding, and a warn tier whose severity surfaces findings without blocking — and the array-sort-comparator class is configured in the error tier ([test](tests/eslint-mirror.compliance.l1.test.ts))
