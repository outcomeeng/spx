...
- ALWAYS: the mirror runs each rule at one of two enforcement tiers — an error tier whose severity blocks `spx validation` on any finding, and a warn tier whose severity surfaces findings without blocking — with the array-sort-comparator, redundant-assertion, object-has-own, and duplicate-import classes configured in the error tier and the unicorn-family classes in the warn tier while their backlog is uncleared ([test](tests/eslint-mirror.compliance.l1.test.ts))
