...
- Given the hook environment contains both `CLAUDE_CODE_SESSION_ID` and `CODEX_THREAD_ID`, when the `session-start` payload has no session id, then SPX uses `CODEX_THREAD_ID` as the session id and writes it to the hook env file per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../../../../36-session.enabler/11-session-frontmatter.pdr.md) ([test](tests/session-id.scenario.l1.test.ts))
