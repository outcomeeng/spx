...
- Given the hook environment contains both `CLAUDE_CODE_SESSION_ID` and `CODEX_THREAD_ID`, when the `session-start` payload has no session id, then SPX uses `CODEX_THREAD_ID` as the session id and writes it to the hook env file, matching the agent selection `spx/21-infrastructure.enabler/54-hooks.enabler/21-hook-event-runner.pdr.md` declares ([test](tests/session-id.scenario.l1.test.ts))
- Given the `session-start` payload contains a session id and the hook environment also contains `CLAUDE_CODE_SESSION_ID`, when the hook runs, then SPX uses the payload session id and writes it to the hook env file ([test](tests/session-id.scenario.l1.test.ts))
- Given the hook resolves a session identity and an env-file path, when the hook writes the env file, then the resolved identity is exported as `SPX_AGENT_SESSION_ID` per [`spx/21-infrastructure.enabler/54-hooks.enabler/32-hook-interface-architecture.adr.md`](../../32-hook-interface-architecture.adr.md) ([test](tests/session-id.scenario.l1.test.ts))
