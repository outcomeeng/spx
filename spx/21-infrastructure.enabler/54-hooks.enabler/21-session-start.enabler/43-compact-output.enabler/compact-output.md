...
- Given `spx hook run session-start` receives a payload whose lifecycle source is `compact` and the hook environment carries `CLAUDE_ENV_FILE` with no `CODEX_THREAD_ID` or `CLAUDE_CODE_SESSION_ID`, when the CLI transport resolves default Claude Code runtime policy, then SPX writes the compact foundation directive to process stdout ([test](tests/compact-output.scenario.l2.test.ts))
