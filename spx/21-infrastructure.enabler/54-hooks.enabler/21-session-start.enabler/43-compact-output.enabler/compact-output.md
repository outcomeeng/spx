...
- Given `spx hook run session-start` receives a payload whose transcript path resolves inside one registered agent's session-store root while the hook environment also carries a different agent's session variables, when the CLI transport resolves compact stdout policy, then SPX applies the payload-classified agent's policy and ignores the environment ([test](tests/compact-output.scenario.l2.test.ts))

### Mappings

- Each registered agent's declared `hooks.sessionStart.compactStdout` default maps a compact-source payload classified as that agent to emitting the compact foundation directive on process stdout when the default is true, and to no process stdout when it is false ([test](tests/compact-output.mapping.l2.test.ts))
