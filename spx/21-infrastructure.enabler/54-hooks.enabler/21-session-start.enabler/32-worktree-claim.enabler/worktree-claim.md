...
- Given the `session-start` hook receives a payload and env-file path without a session identity, when the event runs, then SPX appends the project exports and does not append `SPX_WORKTREE_CLAIM_PATH` ([test](tests/worktree-claim.scenario.l1.test.ts))
- Given the `session-start` hook receives a payload and env-file path with a session identity but cannot resolve a controlling holder process, when the event runs, then SPX records the claim diagnostic and does not append `SPX_WORKTREE_CLAIM_PATH` ([test](tests/worktree-claim.scenario.l1.test.ts))
