...
- Given the `session-start` hook adapter receives a payload whose lifecycle source is `compact`, when the event runs, then SPX emits a foundation re-anchor directive on hook stdout instructing re-invocation of `/understand` then `/contextualize` before resuming spec-governed work ([test](tests/session-start.scenario.l1.test.ts))
