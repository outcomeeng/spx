...
- Given `spx hook run session-start` receives a payload whose lifecycle source is `compact`, when the CLI transport resolves default Claude Code runtime policy, then SPX writes the compact foundation directive to process stdout ([test](tests/hook-cli.scenario.l2.test.ts))
