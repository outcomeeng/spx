...
PROVIDES typed verification-run infrastructure — the `spx verify` lifecycle over deterministic verification-context materialization and an append-only run-journal substrate whose backend binds at the edge
...
- ALWAYS: `spx verify` validates the verification type, scope type, scope identity, and finding payload before appending durable run evidence ([audit])
