...
- ALWAYS: the github-pr backend requires a non-empty repository before it streams — when `GITHUB_REPOSITORY` is absent `append` reports an error rather than a success that silently writes no pull-request comment ([test](tests/github-cli.scenario.l1.test.ts))
