...
- GitHub event name maps to pull-request context: both `pull_request` and `pull_request_target` mark the run as a continuous-integration pull request, and any other event name does not ([test](tests/journal-environment.mapping.l1.test.ts))
...
- ALWAYS: streaming is best-effort once the event is durably appended — a streaming-emit failure does not fail the append, so a retry cannot duplicate a committed event; a missed emit self-heals on the next append or seal, which re-renders the full projection ([test](tests/streaming.scenario.l1.test.ts))
