...
- ALWAYS: under the GitHub pull-request environment, `open` hydrates the pull request's prior runs by calling `hydratePriorRuns` of `spx/21-infrastructure.enabler/43-github-ci.enabler/21-artifact-journal-store.enabler` over the workflow-restored staging directory resolved from the environment snapshot, and reports the run's per-run artifact name from that node's naming for the verification workflow's upload step — materializing prior runs over the injected filesystem and reaching no Actions runtime ([audit])
