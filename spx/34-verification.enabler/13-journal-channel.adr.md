...
Backend selection lives at the edge, read from the environment inside the channel, because where a run persists and streams is a property of the invocation environment, not of the skill or the verb: a developer checkout wants a local file and standard output, a CI pull-request run wants a durable artifact store and a pull-request comment projection. A `--backend` flag threaded through every verb, or a skill that names its backend, would drift across callers and couple each to one environment. The GitHub transport that retains the durable artifact and writes the pull-request comment belongs to the GitHub CI adapter boundaries — the Appendable store of `spx/21-infrastructure.enabler/43-github-ci.enabler/21-artifact-journal-store.enabler` and the Snapshot projection of `spx/21-infrastructure.enabler/43-github-ci.enabler/21-snapshot-adapter.enabler`; the journal command composes those injected capabilities and never owns GitHub API mechanics.
