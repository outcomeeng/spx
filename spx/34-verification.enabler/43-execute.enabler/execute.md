...
- ALWAYS: a present language whose product directory supplies no runner seals the run `interrupted` and the executor's result names the product directory searched, so a product without the runner is distinguishable from a language detection gated out and from a run that started and failed ([test](tests/execute.compliance.l1.test.ts))
