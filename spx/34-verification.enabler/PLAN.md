...
1. Apply the parent `spx/34-verification.enabler/32-verify.enabler` cross-lifecycle assertions: lock the full verb mapping and the journal-event boundary, add the `spx verify` descriptor's L2 CLI test for the stdin and Commander wiring, then remove the parent entry from `spx/EXCLUDE` as its implementation begins passing.
