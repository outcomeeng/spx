...
2. **NEW** `test` **executor**, two parts (see *Execution surface*): (a) register `test` as a verification type in the verify recorder's evidence-validator registry (`spx/34-verification.enabler/32-verify.enabler/13-verify-module-structure.adr.md`) so recording validates its evidence — this part is recorder-side but is **not** a peer of agentic `65-audit`/`65-review` (deterministic, no cross-run finding identity, does not consume `54-run-set-orchestration`); (b) the `spx verification test` executor in the new `src/commands/verification-exec/` that drives vitest and feeds per-case units to the recorder. Node boundaries and indices settled in `/decompose`.
...
- `spx verification <type> <verb>` (`spx verification validate`, `spx verification test`, `spx verification eval`) — the **executor**. spx drives the deterministic runner itself and records each progress unit as it goes.
...
- `src/interfaces/cli/verification.ts` — one descriptor; nouns `run | validate | test | eval`.
- `src/commands/verify/` — the recorder, pure run lifecycle over the journal, **unchanged**.
- `src/commands/verification-exec/` — the executor: drives `src/test/registry.ts` / `src/validation/registry.ts` and feeds each unit to the injected recorder port.

Because execution lives in the exec module and the verify module still only records, `13-verify-module-structure.adr.md`'s invariant "verifier execution remains outside the verify module" stays literally true and needs no amendment.

**Evidence unit is per-type; streaming is an adapter property, never the contract.** The contract is one journal event per progress unit; the unit differs by type — a validation **stage** (`src/validation/registry.ts` already exposes ordered pipeline stages — the `[1/7]…[7/7]` output), a test **case**, an eval **case**, an audit/review **finding**. `verification.md` already asserts a run "streams to its backend incrementally as events append." An adapter that can only emit a terminal payload (vitest's JSON reporter is atomic) appends its units at `finish` — degraded observability under the same contract. Slice 1's vitest atomicity is an adapter fact, not a design premise: validation streams its stages natively today.
...
| `spx/34-verification.enabler/verification.md`                                                 | Deterministic `spx test`/`spx validation` stay top-level; the verify surface records **scoped agentic evidence**; spx never drives a verification agent | **Amend** the top-level-deterministic and agentic-evidence clauses to admit an executor noun for deterministic types. The "never drives an **agent**" clause stands — no executor exists for audit/review.             |
| `spx/34-verification.enabler/32-verify.enabler/13-verify-module-structure.adr.md`             | "Verifier execution remains outside the verify module"; verify commands never spawn or select a verifier agent                                          | **Stands unamended.** Execution lives in `src/commands/verification-exec/`; the verify module still only records.                                                                                                      |
...
Code follows spec: `VERIFY_VERIFICATION_TYPE` (`{audit, review}`) gains `validate`, `test`, `eval` once the specs admit them.

**Blocking gate before the test executor.** `spx/34-verification.enabler/32-verify.enabler/ISSUES.md` requires deciding whether `status`/`render` next actions filter `scope add`/`finding add` by the run type's registered evidence validators **before** a verification type registers only part of the evidence-action surface. Whether the `test` type registers a `finding` validator (a failing test **case** as a finding) or only `scope` decides whether this gate fires — settle it when the type registers.
