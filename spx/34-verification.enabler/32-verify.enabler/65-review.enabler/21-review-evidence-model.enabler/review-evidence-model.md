...
PROVIDES review payload validation over platform-neutral review terminal metadata, reviewed units, and anchored review comments
SO THAT review producers, GitHub backends, local renderers, and other delivery surfaces
CAN validate review terminal metadata and evidence payloads without requiring provider-specific command vocabulary

## Assertions

### Conformance

- Review terminal metadata conforms to the platform-neutral review envelope schema: provider identity when present, actor, state, body, submitted time, commit identity, and optional URL ([test](tests/review-payload.conformance.l1.test.ts))
...
- ALWAYS: review scope, finding, and terminal metadata payloads validate through the shared verification-type evidence-validator registry before journal events append or terminal completion is recorded ([audit])
