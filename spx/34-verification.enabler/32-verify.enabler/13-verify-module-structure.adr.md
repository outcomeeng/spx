...
- Evidence-payload validation dispatches through an evidence-validator registry keyed by verification type and evidence kind; adding a verification type's scope or finding validation adds registry entries and changes no verify dispatch control flow.
...
- ALWAYS: verification-type-specific scope-payload and finding-payload validation dispatches through a statically typed, explicitly enumerated evidence-validator registry keyed by verification type and evidence kind, so `src/domains/verify/` selects validators by registry lookup rather than verification-type-name branching ([audit])
...
- ALWAYS: `src/interfaces/cli/verify.ts` owns Commander registration and process-boundary parsing for the `spx verification run` command family, and imports command handlers rather than domain internals directly ([audit])
...
- NEVER: verify domain or command code branches on a verification-type name to select scope or finding validation; a new verification type adds evidence-validator registry entries ([audit])
