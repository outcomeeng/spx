...
- In a file-scoped audit run, the first audit scope payload conforms only when it records a required root unit with no parent and a subject path equal to the run's normalized file selector; each later child payload conforms only when its parent unit is already recorded, while the child subject may identify a related file ([test](tests/audit-scope.conformance.l1.test.ts))
...
- ALWAYS: audit scope validation preserves root-first and parent-before-child append order across the recorded scope evidence prefix ([test](tests/audit-evidence-validation.compliance.l1.test.ts))
