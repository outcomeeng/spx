...
- ALWAYS: verification-type evidence validation receives the run's canonical scope selector and already recorded evidence prefix before appending, so a validator can enforce subject anchoring and parent-before-child ordering without reading storage itself ([test](tests/verify-payload.compliance.l1.test.ts))
