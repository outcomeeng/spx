...
- ALWAYS: an evidence append rejects when the run's recorded drive mode is spx-driven, so a caller holding the run token cannot add scope or finding evidence to a run spx opens, streams, and seals — the enforcement counterpart of the next-action projection filter ([test](32-evidence-append.enabler/tests/verify-drive-mode.compliance.l1.test.ts))
