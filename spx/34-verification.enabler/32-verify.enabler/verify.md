...
- ALWAYS: `start` records the run's drive mode — caller-driven or spx-driven — so later lifecycle operations distinguish a run a caller appends to from a run spx opens, streams, and seals within one invocation ([audit])
- ALWAYS: status and render next actions filter by the recorded drive mode, so an unsealed spx-driven run advertises no caller evidence-append action such as `scope add` or `finding add` ([audit])
