...
PROVIDES the `spx verify --verification-type <type> --scope-type <scope-type> --scope <scope> --input <input-source> <verb>` command lifecycle for typed verification runs over the verification-context and journal substrate
...
- Given a started run, when `spx verify --verification-type review --scope-type changeset --scope <base>..<head> --input stdin input` runs for its run token, then it returns the exact verification input whose digest was recorded at start ([test](tests/verify-input.scenario.l1.test.ts))
- Given a started run with appended scope and findings, when `spx verify --verification-type review --scope-type changeset --scope <base>..<head> --input stdin finish` runs, then it seals the journal and renders a terminal projection from the event history ([test](tests/verify-lifecycle.scenario.l1.test.ts))
