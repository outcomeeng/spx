...
- Given a sealed review run with terminal completion, when render is requested, then it renders the journal projection including the authoritative finding count and the per-disposition counts — `blocking`, `debt`, `filed`, `stale` — from the event history, listing `filed` and `stale` findings in their own labeled sections apart from `blocking` and `debt`, without appending journal events ([test](tests/verify-render.scenario.l1.test.ts))
...
- ALWAYS: `finish`, `status`, and `render` report the run token and, from the journal projection of every sealed run, audit and review alike, the finding count per disposition — `blocking`, `debt`, `filed`, `stale` — with the authoritative total beside them ([test](tests/verify-status.compliance.l1.test.ts))
