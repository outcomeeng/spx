...
- ALWAYS: `render` projects an unsealed run read-only — reporting `sealed: false` with no terminal status, appending no journal event, and sealing no run — so a started run stays renderable and appendable before `finish` ([test](tests/verify-render.compliance.l1.test.ts))
