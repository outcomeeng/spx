...
4. Evidence validation is reasoned: a scope, finding, or terminal metadata validator returns the accepted value or a reason naming the failing payload field path or the unmet structural requirement, and the command layer renders that reason in a labeled rejection block on standard error composed through `src/lib/terminal-text/`.
5. CLI command vocabulary stays under `spx/60-surfaces.enabler/21-cli-surface.enabler/21-verification.enabler`; this node owns the library and command-layer lifecycle behavior behind that surface.
