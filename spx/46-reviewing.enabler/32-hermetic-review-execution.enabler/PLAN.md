...
- Do not edit `src/lib/process-lifecycle/` or `testing/harnesses/process-lifecycle/` in this packet. If this packet discovers missing lifecycle-runner behavior, stop R2 implementation work, re-read `spx/16-config.enabler/PLAN.md` Open Coordination, and inspect open process-lifecycle PRs and branches. Claim the existing shared CLI/process-lifecycle PR when one exists; if none exists, record a blocker in this PLAN instead of opening a shared branch from R2.
