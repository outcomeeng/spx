...
Before branching, verify `git ls-tree origin/main -- spx/46-reviewing.enabler/21-review-config.enabler/` and `git ls-tree origin/main -- spx/33-agent-environment.enabler/32-runtime-config.enabler/` report the R1 and E2 artifacts. Implement the isolated reviewer execution substrate shared by branch and PR review commands. Prepare execution directories and runtime config before launch, use the shared process lifecycle runner, separate target materialization from reviewer runtime state, and emit metadata needed by review state. Prove invoking-agent state is not mutated, lifecycle signals reap reviewer children, and non-zero or malformed reviewer outputs produce terminal failure state. Open one PR and ask reviewers to audit hermetic boundaries.
```
