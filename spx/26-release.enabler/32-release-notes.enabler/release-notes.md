...
- NEVER: a release whose every commit carries an omitted conventional type reaches the producer — generation fails before the agent runs, because a release with no user-visible change has no notes to write ([test](tests/release-notes.compliance.l1.test.ts))
- ALWAYS: the release-notes prompt carries the release version, the subjects of the release's commits whose conventional type is not `spec`, `test`, `refactor`, `style`, `docs`, `ci`, or `build` — the faithfulness audit judges against those same subjects, so neither the producer nor the audit reads a user-visible effect into a commit the notes omit as declared — and the checked canonical staged artifact path seeded from existing changelog content; instructs the producer to summarize user-visible behavior while omitting process-only changes; and structurally validated, faithfulness-audited staged notes atomically replace the checked canonical changelog path without exposing partial content ([test](tests/release-notes.compliance.l1.test.ts))
...
- ALWAYS: generated release notes describe and group user-visible changes faithfully to the underlying commits, omit spec-only, test-only, release-mechanics, and internal implementation changes that have no user-visible effect, and introduce no claim absent from the commits ([audit])
