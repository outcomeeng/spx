...
- ALWAYS: the release-notes prompt carries the release version, the commit subjects, and the checked canonical staged artifact path seeded from existing changelog content, and validated staged notes promote to the checked canonical changelog path ([test](tests/release-notes.compliance.l1.test.ts))
