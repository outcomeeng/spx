...
- ALWAYS: product context supplies the product specification, governing decisions, and affected specifications through deterministic selection of direct spec-tree changes plus every candidate owner and governing lowest common ancestor for implementation paths across both release-range endpoint trees; absence of a spec tree permits generation from release data, while an unresolved implementation path or failure to read selected context fails before agent invocation ([test](tests/release-notes.compliance.l1.test.ts))
