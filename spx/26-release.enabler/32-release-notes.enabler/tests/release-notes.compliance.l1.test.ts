...
describe("composeReleaseNotes builds the prompt from the release data and resolved configuration", () => {
  it("includes the release version, the commit subjects, and the resolved changelog path as prompt data", async () => {
    await withReleaseNotesEnv(async ({ workingDirectory, readArtifact, canonicalizePath, isSymbolicLink }) => {
      ...
      const expectedResolvedPath = oracleResolvedChangelogPath(workingDirectory, config.changelogPath);
      const conformant = sampleReleaseTestValue(arbitraryConformantChangelog(releaseData.version, subjects));
      const agentRunner = new RecordingWritingAgentRunner(workingDirectory, resolvedPath, conformant);
      ...
      const decodedSubjectBlock = decodeReleaseNotesPromptData(delimitedSubjectBlock);
      ...
      expect(decodedPathBlock).toBe(JSON.stringify(expectedResolvedPath, null, COMMIT_SUBJECTS_JSON_INDENT));
  ...
  it("keeps delimiter-like release version text inside the encoded data block", async () => {
    await withReleaseNotesEnv(async ({ workingDirectory, readArtifact, canonicalizePath, isSymbolicLink }) => {
      ...
      expect(delimitedVersionBlock).not.toContain(RELEASE_VERSION_DATA_BLOCK_CLOSE);
      expect(decodeReleaseNotesPromptData(delimitedVersionBlock)).toBe(
        JSON.stringify(releaseData.version, null, COMMIT_SUBJECTS_JSON_INDENT),
      );
      expect(prompt).not.toContain(`version ${releaseData.version}`);
  ...
  it("keeps instruction-like changelog path text inside the encoded data block", async () => {
    await withReleaseNotesEnv(async ({ workingDirectory, readArtifact, canonicalizePath, isSymbolicLink }) => {
      ...
      const expectedResolvedPath = oracleResolvedChangelogPath(workingDirectory, changelogPath);
      const conformant = sampleReleaseTestValue(arbitraryConformantChangelog(releaseData.version, subjects));
      const agentRunner = new RecordingWritingAgentRunner(workingDirectory, resolvedPath, conformant);
      ...
      expect(delimitedPathBlock).not.toContain(CHANGELOG_PATH_DATA_BLOCK_CLOSE);
      expect(decodeReleaseNotesPromptData(delimitedPathBlock)).toBe(
        JSON.stringify(expectedResolvedPath, null, COMMIT_SUBJECTS_JSON_INDENT),
      );
    });
  });
});

function promptDataBlock(prompt: string, openMarker: string, closeMarker: string): string {
...
describe("composeReleaseNotes keeps the changelog path within the product working tree", () => {
  ...
  it("creates notes at a configured nested path whose parent directory does not exist", async () => {
    await withReleaseNotesEnv(async ({ workingDirectory, readArtifact, canonicalizePath, isSymbolicLink }) => {
      const releaseData = sampleReleaseTestValue(RELEASE_TEST_GENERATOR.releaseData());
      const subjects = releaseData.commits.map((commit) => commit.subject);
      const changelogPath = sampleReleaseTestValue(arbitraryNestedConfiguredChangelogPath());
      const config = { changelogPath };
      const resolvedPath = resolveReleaseNotesPath(workingDirectory, config);
      const conformant = sampleReleaseTestValue(arbitraryConformantChangelog(releaseData.version, subjects));
      const agentRunner = new RecordingWritingAgentRunner(workingDirectory, resolvedPath, conformant);
  ...
  it("rejects a configured changelog path through a symlink that escapes the working tree", async () => {
    await withReleaseNotesEnv(async ({ workingDirectory, readArtifact, canonicalizePath, isSymbolicLink }) => {
      await withTempDir(RELEASE_NOTES_OUTSIDE_TEMP_DIR_PREFIX, async (outsideDirectory) => {
        ...
        await symlink(outsideDirectory, symlinkPath, RELEASE_NOTES_DIRECTORY_SYMLINK_TYPE);
  ...
  it("rejects a final changelog-path symlink with a missing outside target before invoking the agent", async () => {
    await withReleaseNotesEnv(async ({ workingDirectory, readArtifact, canonicalizePath, isSymbolicLink }) => {
      await withTempDir(RELEASE_NOTES_OUTSIDE_TEMP_DIR_PREFIX, async (outsideDirectory) => {
        ...
        const config = {};
        const resolvedPath = resolveReleaseNotesPath(workingDirectory, config);
        const outsideTarget = join(outsideDirectory, DEFAULT_CHANGELOG_PATH);
        const conformant = sampleReleaseTestValue(arbitraryConformantChangelog(releaseData.version, subjects));
        const agentRunner = new RecordingWritingAgentRunner(workingDirectory, resolvedPath, conformant);
        await symlink(outsideTarget, resolvedPath, RELEASE_NOTES_FILE_SYMLINK_TYPE);
...
describe("isPathContained verifies release path containment edge cases directly", () => {
  it("distinguishes parent traversal, root, prefix lookalikes, and absolute escapes", async () => {
    await withReleaseNotesEnv(async ({ workingDirectory }) => {
      ...
      expect(isPathContained(workingDirectory, PATH_CONTAINMENT_PARENT_DIRECTORY)).toBe(false);
      expect(isPathContained(workingDirectory, prefixLookalike)).toBe(true);
      expect(isPathContained(workingDirectory, PATH_CONTAINMENT_ROOT_CANDIDATE)).toBe(true);
      expect(isPathContained(workingDirectory, absoluteEscape)).toBe(false);
    });
  });

  it("rejects a Windows cross-drive candidate", () => {
    const root = win32.join("C:\\", sampleReleaseTestValue(arbitraryPathSegment()));
