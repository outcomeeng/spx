...
- ALWAYS: release filesystem scenarios materialize in isolated real product directories, and cleanup removes staged artifacts and temporary directories on success and failure while preserving the original result ([audit])
- ALWAYS: combinatorial release properties execute controlled dependency cases in memory when real process or filesystem operations would multiply with the generated run count ([audit])
