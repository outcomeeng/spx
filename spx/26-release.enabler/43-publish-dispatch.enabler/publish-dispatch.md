...
- Given a release tag whose commit the product checkout has moved past, when `spx release publish` reads the release artifacts, then the changelog it validates is the one committed at the tagged commit, whatever the checkout's changelog file holds ([test](tests/publish-dispatch.scenario.l1.test.ts))
