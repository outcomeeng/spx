...
- Given the package registry contains the release version under a name, version, or commit that does not match the verified release, when `spx release publish` runs, then publication fails and the GitHub Release remains unchanged; a record matching those but not yet carrying provenance is re-read under the confirmation backoff instead, so a dispatch resumed before the attestation propagates converges rather than failing ([test](tests/publish-dispatch.scenario.l1.test.ts))
