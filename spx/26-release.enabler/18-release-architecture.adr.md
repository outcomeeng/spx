...
- NEVER: the release-data external-operation boundary delegates an executable other than git or a git subcommand outside its source-owned local operation set ([compliance])
...
- ALWAYS: release-data computation imports, constructs, and receives no network or model client; its only external collaborator is the guarded git runner over the local working tree ([audit])
