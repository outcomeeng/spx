...
Once structural parsing is gated on the needle, decoding transcript bytes into text is the cost that still scales with the store — every candidate is decoded so its text can be searched, and the search then rejects nearly all of them. Searching the encoded needle in the undecoded bytes answers the same question without that decode. UTF-8 is self-synchronizing, so a byte sequence encoding a needle occurs in a transcript's bytes exactly when the needle occurs in its decoded text; byte candidacy and text candidacy are the same set, and deferring the decode to byte hits changes cost alone, never results. A branch selector gains the same bound on its evidence reads: a command that associates a branch names it and a record that carries a branch names it, so a transcript whose bytes never name the branch holds no branch evidence and is not decoded, while its opening metadata still answers worktree-root association without any content read.
...
- For every needle and every transcript, the needle's encoded bytes occur in the transcript's bytes exactly when the needle occurs in its decoded text.
- A content or branch selector never decodes a transcript whose bytes carry no needle that selector requires.
...
- ALWAYS: scanning a transcript's undecoded bytes for a needle admits exactly the transcripts whose decoded text contains that needle, for every needle and every transcript content ([property])
- NEVER: a content or branch selector decodes a transcript whose bytes carry no needle that selector requires, in candidate scanning or in branch-evidence collection ([compliance])
...
- ALWAYS: session-store filesystem operations — undecoded byte reads and text decodes alike — session-address resolution, and worktree association dependencies cross typed dependency-injection boundaries ([audit])
