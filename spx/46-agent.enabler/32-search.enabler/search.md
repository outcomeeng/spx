...
PROVIDES coding-agent session search over agent-native transcript stores
...
- Given top-level transcripts from more than one agent under the current product scope, plus a transcript outside that scope, when `spx agent search --pickup-id <id>` runs, then only product-scoped top-level agent sessions whose transcript contains the exact pickup marker are returned ([test](tests/search.scenario.l1.test.ts))
