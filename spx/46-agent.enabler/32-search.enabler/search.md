...
- `--pickup-id <id>` maps to an exact content search for `<PICKUP_ID><id></PICKUP_ID>`, `--contains <literal>` maps to a literal transcript content search, `--session-id <id>` maps to the session whose opening metadata carries that id, located as a needle across every agent store, `--branch <name>` maps to branch association from transcript branch metadata recorded in any transcript record where the agent adapter declares a record reader and in the opening record where it declares none, same-product worktree checkout roots, accepted transcript command evidence, and Codex subagent transcript branch evidence attributed to the parent top-level session, `--agent <kind>` maps to the selected agent adapter set, `--limit <n>` maps to the maximum result count, `--since <duration>` maps to the recent-session bound on transcript file modification time, and `--all` maps to removal of that bound ([test](tests/search.mapping.l1.test.ts))
...
- A session whose opening metadata carries the requested id is returned by `spx agent search --session-id <id>` whichever agent adapter holds it, whichever product the invocation addresses, and whichever working directories its transcript records ([test](tests/session-identity.property.l1.test.ts))
- `spx agent search --session-id <id>` reports a recorded working directory inside the invocation product when the transcript records one, and the session's opening working directory otherwise ([test](tests/session-identity.property.l1.test.ts))
- `spx agent search --since <duration>` admits exactly the sessions whose transcript file modification time falls within that duration, for every duration and transcript age; unlike `spx agent resume`, search decides eligibility from that modification time rather than from a timestamp parsed out of the transcript ([test](tests/reach-window.property.l1.test.ts))
- Absent `--since` and `--all`, `spx agent search` bounds candidates by transcript file modification time to a search-owned thirty-day recent-session window that varies independently of the window `spx agent resume` applies ([test](tests/reach-window.property.l1.test.ts))
...
- NEVER: a session id reaches an agent session store as a path component; the store receives it only as the locator's needle, whatever characters it carries
- NEVER: a locator-dependent selector reads a transcript past its metadata head that the locator did not name for that invocation, in candidate scanning or in branch-evidence collection
- NEVER: a search whose only selector is content reads a transcript's structural session metadata when the locator did not name that transcript for the selector's needle ([test](tests/scan-bound.compliance.l1.test.ts))
...
- NEVER: a selector value that is empty or carries a line terminator reaches an agent session store; the invocation fails with a non-zero diagnostic before any store read
- ALWAYS: when ripgrep cannot be started, `--contains`, `--pickup-id`, `--branch`, and `--session-id` fail with a non-zero diagnostic naming ripgrep before any transcript is read, and a selector-free listing returns its results
...
- ALWAYS: pickup-marker, literal-content, agent-session-id, and agent-kind searches reach every worktree of the invocation's product without a branch selector; a pickup-marker or literal-content search whose agent adapter declares a transcript record reader excludes a top-level session whose recorded working directories all lie outside that product, an agent-kind search excludes one whose opening working directory lies outside it, and an agent-session-id search excludes neither, whichever agent adapter holds the session ([test](tests/search.compliance.l1.test.ts))
