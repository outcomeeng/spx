...
PROVIDES coding-agent session search over Codex and Claude Code agent-native transcript stores
...
- Given a top-level agent session whose transcript branch metadata records the requested branch outside the invocation product scope, when `spx agent search --branch <name> --json` runs, then the session is returned with its recorded current working directory and `branch` in its match reasons ([test](tests/search.scenario.l1.test.ts))
