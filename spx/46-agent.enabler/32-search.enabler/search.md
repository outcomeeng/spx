...
- A session whose agent adapter declares a session-address resolver is returned by `spx agent search --session-id <id>` whichever product the invocation addresses and whichever working directories its transcript records ([test](tests/session-identity.property.l1.test.ts))
...
- NEVER: `spx agent search --session-id <id>` enumerates a session store whose agent adapter declares a session-address resolver ([test](tests/session-identity.compliance.l1.test.ts))
- NEVER: `spx agent search --session-id <id>` resolves a store address for an id carrying a path separator or a parent-directory segment ([test](tests/session-identity.compliance.l1.test.ts))
