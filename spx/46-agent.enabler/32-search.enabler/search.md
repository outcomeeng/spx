...
- ALWAYS: branch-associated search returns only sessions associated with the requested branch through transcript branch metadata, same-product worktree checkout roots, or accepted transcript command evidence ([test](tests/search.compliance.l1.test.ts))
- NEVER: branch existence alone returns a session for `spx agent search --branch <name>` ([test](tests/search.compliance.l1.test.ts))
- NEVER: agent subagent transcripts are returned as branch-associated search results, even when the subagent transcript names the requested branch ([test](tests/search.compliance.l1.test.ts))
