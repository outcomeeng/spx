...
- ALWAYS: a candidate is identified from its transcript's opening metadata, its activity timestamp is identified from a bounded transcript tail scan when present, and its worktree-scope working directory honors working-directory records within the same bounded metadata head and activity tail, never by parsing the whole transcript ([test](tests/resume.compliance.l1.test.ts))
