...
PROVIDES fast discovery and resume launch of recent coding-agent sessions, scoped to the current worktree for every supported agent or to an explicitly recorded branch for agents whose session metadata carries branch identity
SO THAT users working in Git worktrees
CAN continue the right agent conversation through `spx agent resume` after a terminal restart, without recalling which agent or session it was
...
- Given `spx agent resume --branch <name>` runs, when candidates are discovered, then sessions whose agent records an initial branch equal to `<name>` are included regardless of which worktree they ran in, while sessions from agents whose metadata carries no branch identity are excluded ([test](tests/pi-resume.scenario.l1.test.ts))
- Given matching sessions from more than one agent exist, when `spx agent resume` runs in an interactive terminal, then the user can choose one candidate and SPX launches that candidate through the agent's native resume command ([test](tests/pi-resume.scenario.l1.test.ts))
...
- Resume scope maps to candidate set: no scope flag selects every supported agent's sessions in the current worktree; `--branch <name>` selects sessions carrying that initial recorded branch across worktrees and excludes sessions without recorded branch identity ([test](tests/pi-resume.mapping.l1.test.ts))
...
- ALWAYS: each agent's candidate set excludes the transcript kinds that agent's own node declares unresumable ([test](tests/resume.compliance.l1.test.ts))
- ALWAYS: candidate discovery reads each agent's session store from the resolution order that agent's own node declares ([test](tests/pi-resume.compliance.l1.test.ts))
- ALWAYS: explicit activity windows admit a long-running transcript when its bounded tail carries recent activity and exclude it when the newest tail activity is stale, without requiring the opening row to remain in the tail ([test](tests/pi-resume.compliance.l1.test.ts))
