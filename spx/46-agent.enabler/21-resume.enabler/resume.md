...
- Given a Codex session whose opening metadata records a working directory outside the invocation worktree while a working-directory record within the bounded metadata head or activity tail resolves inside the same local worktree root, when `spx agent resume` runs in worktree scope, then the session is included and its candidate working directory is the newest such in-scope record
...
- ALWAYS: worktree-scope classification reads working-directory records within the bounded metadata head and activity tail where the agent adapter declares a working-directory record reader, and includes a session when its opening recorded working directory or any such record resolves inside the invocation worktree root
