...
- Given a Codex session whose opening metadata records a working directory outside the invocation worktree while a working-directory record within the bounded metadata head or activity tail resolves inside the same local worktree root, when `spx agent resume` runs in worktree scope, then the session is included and its candidate working directory is the newest such in-scope record ([test](tests/record-scope.scenario.l1.test.ts))
...
- Resume scope maps to candidate set: no scope flag selects Codex, Claude Code, and Pi sessions in the current worktree; `--branch <name>` selects sessions carrying that initial recorded branch across worktrees and excludes sessions without recorded branch identity ([test](tests/pi-resume.mapping.l1.test.ts))
...
- ALWAYS: a candidate is identified from its transcript's opening metadata, its activity timestamp is identified from a bounded transcript tail scan when present, and its working directory honors working-directory records within the same bounded metadata head and activity tail, never by parsing the whole transcript ([test](tests/resume.compliance.l1.test.ts))
- ALWAYS: worktree-scope classification reads working-directory records within the bounded metadata head and activity tail where the agent adapter declares a working-directory record reader, and includes a session when its opening recorded working directory or any such record resolves inside the invocation worktree root ([test](tests/record-scope.compliance.l1.test.ts))
