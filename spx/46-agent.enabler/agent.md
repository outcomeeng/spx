...
- ALWAYS: agent session coordination resolves Codex and Claude Code homes from `CODEX_HOME` and `CLAUDE_CONFIG_DIR`, and resolves Pi sessions from `PI_CODING_AGENT_SESSION_DIR` before `PI_CODING_AGENT_DIR`, before default home-directory paths across agent session discovery surfaces ([test](tests/agent-home-resolution.compliance.l1.test.ts))
