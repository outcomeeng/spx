...
4. Add a search compliance assertion that full-history branch evidence is raw-prefiltered and only literal-positive files enter structured command or subagent parsing. This turns the observed scan explosion into deterministic work evidence instead of a wall-clock-only regression.
...
7. Refactor branch association into two passes behind injected boundaries:
   - pass one gathers direct metadata and same-product worktree matches for eligible output candidates, and raw-prefilters historical files for the exact requested branch literal;
   - pass two structurally parses command and Codex subagent evidence only from raw-positive files, validates accepted command forms, attributes subagent evidence to top-level sessions, and rejects false-positive raw matches;
...
- Default branch search preserves full-history command and Codex subagent association without structurally parsing literal-negative historical transcripts.
