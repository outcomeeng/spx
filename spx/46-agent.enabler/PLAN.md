...
1. Extend `spx/36-session.enabler/32-session-identity.enabler` and `spx/37-compact.enabler` with Pi session identity supplied by Pi's session runtime or extension event context.
2. Extend `spx/38-worktree.enabler` controlling-process recognition and lifecycle binding so a live Pi process holds the worktree claim for its full lifetime.
3. Extend `spx/33-harness-environment.enabler` with Pi instruction, plugin, skill, hook, and runtime-configuration reconciliation.
4. Extend `spx/54-diagnose.enabler` with Pi installation and configured-environment readings.
5. Supply `spx/57-methodology-lifecycle.enabler` with the exact native session identity its migration resume, verification evidence, and closure bind to.
...
4. Add search compliance assertions that full-history branch association is served from a versioned persistent derived index in a shared product-state scope. Every source adapter exposes opaque resumable discovery state and advances it within separate directory-entry, metadata-read, transcript-byte, and deadline budgets; constructing one page may not enumerate, sort, or stat the full store. Query-time maintenance processes only the returned bounded delta of created, changed, or removed transcripts. Unchanged historical transcripts are never reopened during a warm query, and incomplete discovery or derivation coverage is reported explicitly instead of producing a false no-match result.
...
4. Extend the search filesystem/scanner evidence with observations for discovery pages and cursors, directory entries visited, metadata reads, created/changed/removed identities, index hits, transcript bytes examined, raw-prefilter candidates, full transcript reads, and structurally parsed rows. On a generated high-volume indexed history, assert that a warm branch query performs no full-store listing or stat pass and reads no unchanged historical transcript content. After adding literal-negative and JSON-escaped positive evidence, assert that maintenance stays within its source-owned entry, metadata, transcript-byte, and deadline budgets, parses only prefilter-positive files, and indexes the escaped branch correctly. A cold history larger than one maintenance budget reports incomplete coverage without returning a definitive empty result, and repeated cursor advances reach complete coverage without restarting discovery from the first entry. Deleting the sole evidence transcript removes its association after bounded removed-file reconciliation. Changing the source-owned index format or command-derivation version invalidates prior coverage and rebuilds it within the same budgets.
...
7. Build a persistent branch-evidence index behind injected boundaries and address it through the shared state API:
   - the index header records source-owned format and command-derivation versions, one opaque discovery cursor and generation per adapter/store root, the completed coverage watermark, and whether the current generation is complete; a version mismatch retires prior derived entries and starts incomplete coverage;
   - each entry records the adapter, canonical transcript identity, observed size and modification time, last-seen discovery generation, attributed top-level session identity, and structurally verified branches;
   - each adapter implements a resumable `discoverTranscriptChanges(cursor, budgets)` boundary that returns only a bounded page of created, changed, unchanged, and removed identities plus the next cursor. The implementation cannot materialize, sort, or stat the complete store to produce that page; directory-entry visits and metadata reads consume explicit budgets;
   - bounded maintenance raw-prefilters only created or changed identities within source-owned transcript-byte and deadline budgets, structurally validates only positive files, tombstones removed identities, and atomically advances discovery and derivation coverage without reopening unchanged historical content;
   - completion of one discovery generation removes entries not observed in that generation; while a generation or derivation rebuild is incomplete, unconfirmed prior entries cannot produce a definitive empty result or an unqualified branch association;
...
- A warm default branch search preserves indexed full-history command and Codex subagent association without a full-store directory/stat pass or any read of unchanged historical transcript content.
- Bounded index maintenance never exceeds its source-owned directory-entry, metadata-read, transcript-byte, or deadline budgets; incomplete coverage is explicit and repeated cursor advances reach complete coverage.
- Deleted transcripts lose their derived associations, and index-format or command-derivation version changes cannot reuse stale evidence.
