...
Bootstrap configured plugin marketplaces, plugins, and skills for supported configured agents.
...
Can proceed after `spx/33-agent-environment.enabler/32-runtime-config.enabler/runtime-config.md` declares the configured-agent config descriptor shape. Installation actions should be separate from status reporting.
...
Before branching, verify `git cat-file -e origin/main:spx/33-agent-environment.enabler/agent-environment.md` succeeds for E0 and `git cat-file -e origin/main:spx/33-agent-environment.enabler/32-runtime-config.enabler/runtime-config.md` succeeds for the E2 configured-agent config artifacts. Implement deterministic status and dry-run planning for configured plugin marketplaces, plugins, and skills. Separate local status reporting from install or update actions. Keep core status offline-capable and record exact installed versions or digests when the configured agent exposes them. Prove installed, missing, stale, failed, dry-run actions without writes, offline mode, and malformed configured entries. Open one PR and ask reviewers to audit offline guarantees and action/status separation.
```
