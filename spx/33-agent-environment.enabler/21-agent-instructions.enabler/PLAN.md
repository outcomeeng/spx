...
This depends on the harness environment descriptor shape and E2 configured-agent config so instruction targets come from configured agents rather than a hardcoded agent list. It can proceed independently from plugin bootstrap once those APIs are stable.
