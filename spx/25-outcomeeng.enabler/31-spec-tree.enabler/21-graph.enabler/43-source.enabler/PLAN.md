...
SPX must not parse implementation files. It parses Spec Tree Markdown only to extract `[test](...)` links from spec nodes. Language-specific providers supply source facts from established tooling.
...
```text
spec assertion -> Markdown [test](...) link -> linked test file -> provider facts -> source ownership classification
```
...
3. Implement the first slice in TypeScript with SPX parsing only Markdown test links and consuming provider-style facts through injected provider fixtures.
...
- SPX parses Spec Tree Markdown for `[test](...)` links and does not parse implementation source files.
