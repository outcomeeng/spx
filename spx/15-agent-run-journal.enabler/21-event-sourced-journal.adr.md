...
The journal owns sequence allocation and admits concurrent appenders to one stream at its own layer: appenders compete for the next sequence, and the store settles each contest by publishing one sequence at most once. A consumer lifecycle above the journal may require one sequential driver per run for its own check-then-act rules; that requirement belongs to the consumer, and the journal neither enforces nor relaxes it. A consumed-sequence rejection proves that another appender published, so the journal allocates again from the refreshed history instead of failing the appender; a seal recorded meanwhile takes precedence and ends the append under the terminal seal, and a rejection the refreshed history cannot explain is a backend defect and surfaces unchanged. Splitting one run's sequence space per worktree or per session is rejected because it breaks the contiguity every replay and cursor rests on.
