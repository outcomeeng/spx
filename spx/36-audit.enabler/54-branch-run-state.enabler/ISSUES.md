...
- `src/commands/audit/run-state.ts`: if `writeTerminalAuditRunState` writes the temporary state file and final rename fails, the same-directory `.state-{id}.tmp` file remains as interrupted-run debris. Decide whether to remove it or document the debris explicitly before expanding terminal-state cleanup semantics.
...
- `src/commands/audit/run-state.ts`: `readAuditBranchRuns` currently reads run state directories sequentially. Consider parallel reads if branch run history retention grows beyond low local counts.
