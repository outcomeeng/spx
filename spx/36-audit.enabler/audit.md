...
- Given an audit run persists state, when the branch slug is known, then the state is written under `.spx/audit/{branch-slug}` at the Git common-dir product root ([review])
...
- ALWAYS: resolve `.spx/audit/` relative to the Git common-dir product root, not the worktree root ([review](../15-worktree-resolution.pdr.md))
