...
[21-audit-module-structure.adr.md](21-audit-module-structure.adr.md) opens by claiming the audit domain follows the three-layer composition of [spx/14-cli-composition.adr.md](../14-cli-composition.adr.md) — "pure modules under `src/domains/audit/`" — yet in the same sentence assigns `src/domains/audit/run-state.ts` "run-directory creation, terminal `state.json` writing" (invariant: "Each exclusive-created audit run directory has exactly one terminal state writer"). Creating directories and writing `state.json` are direct filesystem operations in a domain module, which contradicts `spx/14-cli-composition.adr.md`: "NEVER: a module under `src/domains/{domain}/` … accesses the filesystem or process." The ADR is both internally inconsistent (claims purity, assigns I/O) and in violation of the governing product ADR.

The conforming pattern is the release-data model: filesystem access flows through an injected dependency (as `computeReleaseData` delegates git access to the injected `GitDependencies`) or lives in a `src/commands/audit/` handler, leaving only the pure run-state transition logic — branch identity, slugging, run-id ordering, latest-terminal selection — in the domain.
