...
describe("audit CLI lifecycle commands", () => {
  ...
  it("exits cleanly when an audit status stdout consumer closes the pipe", async () => {
    const harness = await createAuditHarness();
    try {
      const audit = execa(
        NODE_EXECUTABLE,
        [
          CLI_PATH,
          AUDIT_CLI.commandName,
          AUDIT_CLI.statusCommandName,
          AUDIT_CLI_FLAG.BRANCH,
          branch,
          AUDIT_CLI_FLAG.JSON,
        ],
        { cwd: harness.productDir, reject: false },
      );
      expect(audit.stdout).not.toBeNull();
      audit.stdout?.destroy();
