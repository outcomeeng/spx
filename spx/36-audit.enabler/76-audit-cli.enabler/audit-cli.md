...
- Given an audit branch CLI override or audit init head-SHA override is empty, when `spx audit init`, `status`, or `list` receives the empty override, then the command rejects the override without creating a run journal ([test](tests/audit-cli.scenario.l1.test.ts))
...
- Given `--verdict-path` is supplied to `spx audit close`, when the command seals the run journal, then the terminal run state preserves the supplied verdict path and status output can recover it from the journal ([test](tests/audit-cli.scenario.l1.test.ts))
