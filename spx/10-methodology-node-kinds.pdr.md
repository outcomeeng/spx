...
Outcome Engineering durable maps express structural roles through valid node types. This product admits `.enabler` and `.outcome` as node types; substrate, capability, domain, interface, and surface are structural roles carried by product specs and decisions. Reserved role suffixes for substrate, domain, interface, and surface require canonical suffix readiness; capability is a methodology role without a reserved future node suffix.
...
2. Substrate, capability, domain, interface, and surface roles are named by specs and decisions; reserved role suffixes for substrate, domain, interface, and surface have no node-type containment rule without canonical suffix readiness.
...
- ALWAYS: node-kind validation rejects `.substrate`, `.domain`, `.interface`, or `.surface` suffixes without canonical suffix readiness for that suffix ([compliance])
