...
- Given a worktree with a claim-lock recovery marker whose owner matches the live acquisition requester, when that requester attempts acquisition again, then acquisition reports an in-progress claim and leaves the recovery marker unchanged ([test](tests/occupancy-store.scenario.l1.test.ts))
- Given a worktree whose claim-acquisition marker names a live operation process, when another acquisition attempts to mutate the claim, then acquisition reports an in-progress claim and leaves the claim and marker unchanged ([test](tests/occupancy-store.scenario.l1.test.ts))
...
- Given a worktree whose claim-acquisition marker names a live operation process, when the holder releases, then release reports an in-progress claim and leaves the claim and marker unchanged ([test](tests/occupancy-store.scenario.l1.test.ts))
- Given a release removes the claim but leaves a claim-acquisition marker that names a live operation process, when acquisition attempts to claim the worktree, then acquisition reports an in-progress claim and leaves the marker unchanged ([test](tests/occupancy-store.scenario.l1.test.ts))
- Given a worktree claim belongs to a live holder and a claim-acquisition marker names a dead operation process, when the holder releases through another operation, then the marker is recovered and the claim is absent ([test](tests/occupancy-store.scenario.l1.test.ts))
...
- ALWAYS: a claim diagnostic reporting a failed filesystem operation escapes the caught error's message where the store joins it to the store's own error code, so the code survives verbatim and the message reaches no terminal as the bytes it carried ([test](tests/occupancy-store.compliance.l1.test.ts))
