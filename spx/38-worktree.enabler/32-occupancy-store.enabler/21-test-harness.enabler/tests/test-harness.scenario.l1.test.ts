...
import { OCCUPANCY_FS_TEXT_ENCODING } from "@/domains/worktree/occupancy-store";
...
describe("recording OccupancyFileSystem double", () => {
  it("records each operation in invocation order with its op tag and path arguments", async () => {
    await withTempDir(segment(), async (root) => {
      ...
      const link = join(subdirectory, segment());
      ...
      await recording.symlink(target, link);
      await recording.readlink(link);
      await recording.readFile(target, OCCUPANCY_FS_TEXT_ENCODING);
      ...
      expect([...recording.calls]).toEqual([
        ...
        { op: OCCUPANCY_FS_OP.SYMLINK, paths: [target, link] },
        { op: OCCUPANCY_FS_OP.READLINK, paths: [link] },
  ...
  it("delegates each operation to the backing filesystem so its effect is observable", async () => {
    await withTempDir(segment(), async (root) => {
      ...
      expect(await recording.readlink(link)).toBe(target);
      expect(await recording.readFile(target, OCCUPANCY_FS_TEXT_ENCODING)).toBe(content);

      await recording.rm(link);
      await recording.rm(target);
      await expect(recording.readFile(target, OCCUPANCY_FS_TEXT_ENCODING)).rejects.toThrow();
    });
  });
});
