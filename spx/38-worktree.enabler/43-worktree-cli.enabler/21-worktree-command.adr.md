...
`spx worktree` exposes `claim`, `session-start`, `status`, and `release` as a Commander domain registered through the static descriptor registry. `claim` records the holding agent's controlling process — resolved by an explicit `SPX_WORKTREE_CONTROLLING_PID` override, else by walking spx's process ancestry to the nearest ancestor whose command names a known agent runtime (`claude` or `codex`), else the immediate parent — and writes nothing to stdout. `session-start` is the direct hook alternative: it reads the hook JSON payload from stdin, resolves the session id from `payload.session_id`, then `CODEX_THREAD_ID`, then `CLAUDE_SESSION_ID`, writes the hook env file when one is configured, and performs the same claim once from the payload's product directory. `status` reports occupied, unclaimed, or stale for zero or more target worktrees through text output and a machine-parseable `--format json`. `release` removes the running worktree's claim quietly. Claim, session-start, release, and status resolve their target worktree's identity through the same git worktree-root resolution: `claim`, `session-start`, and `release` from the running or payload directory, and `status` from each path argument or, when omitted, the running directory. The process table, the host, the environment, hook stdin, and env-file path are injected.

## Rationale

The SessionStart hook can invoke `spx worktree claim` as a grandchild of the agent (agent → hook → spx) and pass only `--session-id`, so spx must discover the controlling process itself. The agent runtime is the process whose liveness means "the worktree is held," and no universal agent-pid environment variable exists across runtimes, so the agent is identified among spx's ancestors by its command name. An explicit `SPX_WORKTREE_CONTROLLING_PID` override lets an atypical invocation chain — an extra wrapper or shell between the agent and the hook — pin the pid deterministically; the immediate-parent fallback keeps a claim from failing when no agent ancestor is recognized. The claim writes nothing to stdout because the hook injects a command's stdout into the agent's context.

`session-start` exists as an additive hook boundary for runtimes whose hook integration can call `spx` directly. The stable hook inputs — stdin payload, environment, working directory, and env-file path — are consumed in one process, the claim is attempted once, and the env file receives the session/product exports plus `CLAUDE_WORKTREE_CLAIMED` as the claim result. The existing Python hook path remains compatible while the direct path avoids a PreToolUse status-then-claim repair loop.
...
- A successful `session-start` produces no stdout bytes.
- claim, session-start, release, and status resolve their target worktree's identity through the same git worktree-root resolution, so a worktree's claim key is independent of which path inside it names it.
...
- ALWAYS: the session-start handler takes hook stdin content and the env-file path as injected parameters; the descriptor owns process stdin and standard stream behavior, per [`spx/14-cli-composition.adr.md`](../../14-cli-composition.adr.md) ([audit])
- ALWAYS: session-start claim attempts run once per command invocation and reuse the same claim handler and worktree-root resolution as `spx worktree claim` ([audit])
...
- NEVER: session-start performs a status-then-claim repair loop; occupancy repair belongs to the one SessionStart claim attempt and the later on-demand status command ([audit])
