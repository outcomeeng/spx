...
- ALWAYS: controlling-process resolution and every worktree command handler take each boundary it uses — process table, host provider, environment, claim filesystem, git resolver, path-info probe, or random-bytes source — as an injected parameter ([audit])
...
- NEVER: a worktree command handler reads the process table, host, environment, filesystem, git resolver, path info, or random bytes except through its injected dependencies ([audit])
- NEVER: the worktree command descriptor exposes an agent lifecycle hook event as a subcommand ([audit])
- NEVER: framework-level module replacement substitutes for the process table or filesystem — tests inject a controlled table and exercise the real resolution and rendering code paths ([audit])
