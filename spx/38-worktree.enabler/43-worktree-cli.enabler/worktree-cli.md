...
- Given no session identity resolves from `--session-id`, `CLAUDE_CODE_SESSION_ID`, `CODEX_THREAD_ID`, or `SPX_AGENT_SESSION_ID`, when the release handler runs, then the handler reports failure ([test](tests/worktree-cli.scenario.l1.test.ts))
