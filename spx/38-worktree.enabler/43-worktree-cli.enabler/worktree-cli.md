...
- ALWAYS: `spx worktree status [worktree...] --format json` executed through `node bin/spx.js` writes parseable JSON naming each first-seen resolved worktree's occupancy status and exits 0 when at least one worktree is reported, so `/pickup` can branch on occupied, unclaimed, or stale ([test](tests/worktree-cli.compliance.l2.test.ts))
