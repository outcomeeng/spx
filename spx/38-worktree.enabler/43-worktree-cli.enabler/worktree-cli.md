...
PROVIDES Commander.js bindings for `spx worktree status [worktree...]`, `spx worktree claim --session-id <id>`, `spx worktree session-start`, and `spx worktree release`, with shared worktree-root resolution, controlling-process resolution, hook SessionStart payload parsing, hook env-file writing, the silent-stdout claim contract, machine-parseable status output, and exit codes the marketplace consumers depend on
SO THAT the SessionStart hook, `/handoff`, and `/pickup`
CAN claim, query, and release worktree occupancy as a subprocess with predictable output and exit codes, while a direct SessionStart invocation can establish occupancy once without a Python repair loop

## Assertions

### Scenarios

- Given a SessionStart hook payload with a session id and product directory, when the session-start handler runs from that worktree with a resolvable controlling process and hook env file, then it writes one occupancy claim and appends `CLAUDE_SESSION_ID`, `CLAUDE_PROJECT_DIR`, `PROJECT_DIR`, and `CLAUDE_WORKTREE_CLAIMED=1` exports to the env file ([test](tests/worktree-session-start.scenario.l1.test.ts))
- Given a SessionStart hook payload without a session id and `CODEX_THREAD_ID` in the environment, when the session-start handler runs with a hook env file, then it uses the Codex thread id as the recorded session id and exported `CLAUDE_SESSION_ID` value ([test](tests/worktree-session-start.scenario.l1.test.ts))
...
- ALWAYS: a successful `spx worktree session-start` executed through `node bin/spx.js` reads the hook JSON payload from stdin, writes no stdout, exits 0, records the claim once, and writes the hook env file when `$CLAUDE_ENV_FILE` or `--hook-env-file` names one ([test](tests/worktree-session-start.compliance.l2.test.ts))
