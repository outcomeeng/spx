...
- ALWAYS: a successful `spx worktree session-start` executed through `node bin/spx.js` reads the hook JSON payload from stdin, writes no stdout, exits 0, records the claim once, and writes the hook env file when `$CLAUDE_ENV_FILE` or `--env-file` names one ([test](tests/worktree-session-start.compliance.l2.test.ts))
