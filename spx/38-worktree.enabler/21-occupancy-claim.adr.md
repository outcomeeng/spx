...
The claim names its holder by agent and agent session together because an agent session id is opaque and two agents can mint identifiers of the same shape, so the pair is what a reader in another worktree needs to say who holds this one and what a release must match before removing it. Both are facts the claiming invocation already holds — the agent it classified and the session identity it resolved — so recording them adds no lookup and no second identifier for SPX to mint or reconcile.
...
- ALWAYS: release removes only a claim whose agent, agent session id, host, controlling-process id, and start time match the releasing holder ([audit])
