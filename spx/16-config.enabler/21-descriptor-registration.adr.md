...
- Descriptor placement is deterministic: a companion module is any module inside `src/<domain>/config/`, or inside `src/<domain>/` scoped exclusively to config concerns, other than the descriptor module itself — validator helpers, shared-primitive adapters, accessor modules, literal modules, write-back modules, types-only modules, and descriptor-local test generators all count; a descriptor with at most one companion module uses `src/<domain>/config.ts`, and one with two or more uses `src/<domain>/config/descriptor.ts`.
