...
- Given no product config exists, when config resolves through the production registry, then the resolved config includes top-level `methodology` defaults with source `outcomeeng/spec-tree` and version `installed` ([test](tests/methodology-config.scenario.l1.test.ts))
