...
- ALWAYS: `methodology.version` and `methodology.migratingFrom` accept `MAJOR.MINOR` or `MAJOR.MINOR.PATCH` strings and resolve to one exact methodology version; an omitted patch resolves to zero, so `4.0` resolves to `4.0.0` ([test](tests/methodology-config.compliance.l1.test.ts))
