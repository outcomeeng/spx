...
**Resolution:** migrate `src/validation/literal/index.ts` to consume `applyPathFilter` from `@/config/primitives`, keeping any literal-specific output shaping layered on top, so one prefix-matching implementation serves every production domain.
