...
The output sentinel file for each packet must match the node directory slug, such as `canonical-descriptor-digest.md` for `54-canonical-descriptor-digest.enabler`, unless the node-local PLAN names a different sentinel path.
