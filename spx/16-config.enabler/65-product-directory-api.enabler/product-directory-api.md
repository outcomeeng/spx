...
- ALWAYS: the Git common-dir product-root resolver returns the local worktree root as a required `worktreeRoot` field so a caller needing both roots reads `--show-toplevel` once ([test](tests/product-directory-api.compliance.l1.test.ts), [review])
- NEVER: a local worktree-root resolver result carries a `worktreeRoot` field — `detectWorktreeProductRoot` returns the base `GitProductDirResult` whose `productDir` is the worktree root ([test](tests/product-directory-api.compliance.l1.test.ts), [review])
