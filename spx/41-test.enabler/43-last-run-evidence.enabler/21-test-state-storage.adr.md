# Test State Storage Architecture

The testing last-run state module defines the immutable `TestRunState` schema (matching `spx/41-test.enabler/43-last-run-evidence.enabler/11-last-run-file.adr.md`), the per-worktree storage-location helpers keyed on `productDir`, and the read/select surface for terminal runs, with all filesystem dependencies injected. It exports `TestRunState`, the injected `TestRunStateFileSystem` surface (mkdir, writeFile, appendFile, readFile, readdir), `testingRunsDir(productDir)` resolving `.spx/worktree/test/runs/` under the local worktree root, `createTestRunFile(productDir, options)`, `writeTerminalTestRunState(runFilePath, state, options)`, `readTestingRuns(productDir, options)` partitioning terminal from incomplete runs, and `selectLatestTerminalTestRunForNode(runs, nodeTestPaths)` (the latest terminal run whose outcomes cover the node, by `completedAt`, then `startedAt`, then run file name). The write protocol that publishes the JSONL run file is governed by `spx/41-test.enabler/43-last-run-evidence.enabler/32-terminal-write-protocol.adr.md`; root-resolution vocabulary follows `spx/16-config.enabler/65-product-directory-api.enabler/product-directory-api.md`.
...
- ALWAYS: `TestRunState` records branch name, head SHA, and the four staleness-input digests (config, path-set, content, product inputs); staleness comparison uses only those four digests per `spx/41-test.enabler/43-last-run-evidence.enabler/43-staleness-comparison.adr.md`, while branch name and head SHA remain recorded identity fields rather than staleness inputs ([audit])
- ALWAYS: storage helpers resolve `.spx/worktree/test/` under the `productDir` worktree root per `spx/41-test.enabler/43-last-run-evidence.enabler/11-last-run-file.adr.md` ([audit])
