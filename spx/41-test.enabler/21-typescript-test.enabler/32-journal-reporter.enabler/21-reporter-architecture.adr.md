...
- ALWAYS: the journal-streaming run is a `TestingLanguageDescriptor` operation the language-neutral executor reaches through `src/test/registry.ts`, so orchestration names no language per `spx/19-language-registration.adr.md` ([audit])
