...
- ALWAYS: the reporter awaits each sink append before its hook returns, so a hook never returns over a pending append; ordering across hooks Vitest dispatches concurrently from parallel workers belongs to the executor's single-writer sink of `spx/34-verification.enabler/43-execute.enabler`, not to the reporter ([test](tests/journal-reporter.compliance.l1.test.ts))
- ALWAYS: when a sink append rejects during a streaming run, the run rejects with that first failure after the Vitest run returns and yields no terminal status, so a run that lost an append is sealed by the executor as interrupted rather than reporting a terminal status over an incomplete journal ([test](tests/journal-reporter.compliance.l1.test.ts), [test](tests/journal-reporter.scenario.l2.test.ts))
- ALWAYS: the reporter is registered on a programmatically started Vitest run through the Node API, not selected by a `--reporter` command flag, so the run exposes its lifecycle hooks to the reporter ([test](tests/journal-reporter.compliance.l1.test.ts), [test](tests/journal-reporter.scenario.l2.test.ts))
