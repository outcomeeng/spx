...
PROVIDES the typescript Vitest-runner fixtures — a recording command runner that captures constructed commands and returns a configured outcome, a real repo-rooted command runner over a temp project materialized from committed fixture suites, and a fast-check generator for typescript test paths and runner inputs
...
- Given a committed Vitest fixture suite, when `withTempVitestProject` runs a callback, then the suite is copied into a fresh temp project under the OS temp root rather than the repository, the project holds exactly the copied suite, and the project is removed after the callback settles ([test](tests/test-harness.scenario.l1.test.ts))
...
- ALWAYS: the real Vitest command runner executes from the repository root with the temp project isolated by the adapter's `--root` flag, relying on the environment to provide the toolchain ([audit])
