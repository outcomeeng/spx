...
Before applying this plan to agent-facing test selection or output guidance, read `spx/12-agent-harness.pdr.md` and use its vocabulary as the authority: agent harness, agent, agent adapter, and agent session. Treat nearby `agent`, `runtime`, `session`, `Claude`, or `Codex` wording as lower-layer/local vocabulary until reconciled; command vocabulary such as agent-oriented output must stay distinct from agents, agent adapters, and agent sessions.
...
- **Tree placement:** own child enabler at
  `spx/41-test.enabler/90-targeted-execution.enabler`, consuming the language
  adapters (`21-typescript-test`, `21-python-test`), passing-scope config
