...
- Given an operand naming the product root, when target scope is resolved, then every discovered test file is selected whether or not the recursive flag is set, because the product root encloses the whole tree and narrows nothing ([test](tests/targeted-execution.scenario.l1.test.ts))
