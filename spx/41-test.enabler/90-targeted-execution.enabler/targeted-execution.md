...
- Given an empty operand, an absolute operand outside the product root, or a relative operand climbing out of it, when target scope is resolved, then the operand is reported as unresolved, because none names a path inside the product ([test](tests/targeted-execution.scenario.l1.test.ts))
