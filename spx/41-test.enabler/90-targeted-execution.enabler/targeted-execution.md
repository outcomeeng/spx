...
- Given an operand spelling the product root relatively — a bare dot, with or without trailing separators — when target scope is resolved, then every discovered test file is selected whether or not the recursive flag is set, because the product root encloses the whole tree and narrows nothing ([test](tests/targeted-execution.scenario.l1.test.ts))
- Given an empty operand or an absolute operand, when target scope is resolved, then the operand is reported as unresolved, because neither spells a product-root-relative path even when it normalizes to the same form the product root does ([test](tests/targeted-execution.scenario.l1.test.ts))
