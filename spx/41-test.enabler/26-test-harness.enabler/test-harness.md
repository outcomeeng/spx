...
- ALWAYS: `observeRecordingCommandRunner` parametrizes over a language harness's recording-runner factory and that language's source-owned presence and exit-code inputs, driving the generated invocations through the runner without spawning a process and returning the reported presence, per-call exit codes, and recorded calls — so the python and typescript runner test-harness nodes assert their parallel recording runners in their own linked tests over one shared observation ([audit])
