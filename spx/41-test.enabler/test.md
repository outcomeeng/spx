...
- Given a caller requests one node's outcome through the registry-based per-node run, when matching tests exist, then that node's tests execute through the registered runner for each matching extension and fresh last-run evidence is recorded ([test](tests/execution-recording.scenario.l1.test.ts))
