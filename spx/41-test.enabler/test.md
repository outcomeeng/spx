...
- Given `spx test --changed --staged` runs while the worktree has unstaged tracked or untracked files, then it rejects before runner execution ([test](tests/test.scenario.l1.test.ts))
