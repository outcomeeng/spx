...
- Given `spx test --changed --staged` sees staged config changes, when the worktree config differs from the index, then it resolves testing config from the staged snapshot ([test](tests/test.scenario.l1.test.ts))
