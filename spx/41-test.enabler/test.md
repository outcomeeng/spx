...
- Given `spx test --changed --staged` runs while a staged changed path has additional worktree edits, an unstaged/untracked test file falls under the selected test target from changed-set or explicit operands, or a product-input change selects the recursive root while a spec-tree test file is dirty, then it rejects before runner execution; unrelated dirty files outside the selected execution scope do not prevent runner execution ([test](tests/test.scenario.l1.test.ts))
