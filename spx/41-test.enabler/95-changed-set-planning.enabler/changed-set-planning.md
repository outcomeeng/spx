...
- Given a changed path whose name-only diff record contains path whitespace, when `--changed` reads changed paths, then the path is preserved exactly ([test](tests/changed-set-planning.scenario.l1.test.ts))
...
- Given a changed product config file, when `--changed` resolves the affected set, then the full spec test tree is selected recursively ([test](tests/changed-set-planning.scenario.l1.test.ts))
