...
The `--changed` planner resolves its affected test set by diffing the worktree against a base ref that defaults to `origin/<default-branch>` resolved through the shared `src/git/root.ts` primitives, partitioning each changed path into one of two routes — a changed spec or test file under `spx/<node>/` becomes a node-path or test-file operand by path alone, and a changed source file is resolved to its related test file paths by the registered language adapter's related-test capability reached only through `src/test/registry.ts` — and feeds the deduplicated operand union into the `spx/41-test.enabler/90-targeted-execution.enabler` selection so `--changed` is one more operand source, never a parallel dispatch. A registered language whose adapter declares no related-test capability contributes nothing from its changed source files, and that degradation is reported rather than silently dropped.

## Rationale

A changed spec or test file has a direct path relation to the tests it affects — the node's own `tests/` — so pure path math turns it into an operand exactly and without invoking any language tool. A changed source file has no path relation to the tests that exercise it, so its related tests come from a language-native resolver (TypeScript via `vitest list --filesOnly --related`, which emits related test file paths) reached through the registry so the planner names no language per `spx/19-language-registration.adr.md`. Because the related-test capability resolves to test file *paths* rather than running tests, its output is operands the `spx/41-test.enabler/90-targeted-execution.enabler` selection already consumes, so runner selection, runner environment, passing-scope policy, agent output, and last-run recording stay identical to a full run and to an explicit-operand run, with no parallel dispatch path to drift. Defaulting the base to `origin/<default-branch>` through the existing `resolveDefaultBranch` and `resolveRefSha` primitives — the same `origin/HEAD` resolution the rest of the product uses — avoids a second base-resolution implementation.

The layering follows `spx/14-cli-composition.adr.md`: the changed-path partition is a pure function under `src/domains/test/` that takes the changed path set and returns operands plus the source files to route; the planner orchestration under `src/commands/test/` composes the injected git runner, the base-ref primitives, and the registry's related-test resolvers and returns its result; the `src/interfaces/cli/` descriptor owns the `--changed`/`--base` option surface and the process boundary.
...
- The resolved operand set is the deduplicated union of the path-derived node operands and the adapter-resolved related-test paths; the order and repetition of changed paths do not change it.
- A changed path under a node's `spx/<node>/tests/` or matching a node's spec yields an operand by path alone, with no language tool invoked.
...
- ALWAYS: a changed spec or test file under `spx/<node>/` yields a node-path or test-file operand by path alone, with no language tool invoked ([mapping])
...
- ALWAYS: the related-test capability resolves source files to test file paths through an injected command runner, and base-ref resolution reuses `resolveDefaultBranch` and `resolveRefSha` over the injected git runner — never a second default-branch implementation ([audit])
- ALWAYS: the resolved operand set feeds the `spx/41-test.enabler/90-targeted-execution.enabler` selection unchanged, so `--changed` alters only the selected set while runner adapters, runner environment, passing-scope policy, agent output, and last-run recording stay identical to a full run ([audit])
- NEVER: `vi.mock()`, `jest.mock()`, or `memfs` substitutes for the git runner, command runner, or filesystem — tests inject controlled implementations and exercise the real code paths ([audit])
