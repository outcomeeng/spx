...
PROVIDES the python test-runner fixtures — a recording command runner that captures constructed commands and returns a configured outcome, a real `uv`-rooted pytest command runner over a temp project materialized from committed fixture suites, a fast-check generator for python test paths and runner inputs, and the expected pytest product-input path enumeration held independently of the descriptor
...
- Given a committed pytest fixture suite, when `withTempPytestProject` runs a callback, then the suite is copied into a fresh temp project under the OS temp root rather than the repository, the suite path resolves inside that project, and the project is removed after the callback settles ([test](tests/test-harness.scenario.l1.test.ts))
