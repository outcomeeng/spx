...
- Given a project where language detection reports Python absent, when `spx test` runs, then pytest is not invoked ([test](tests/python-test.scenario.l1.test.ts))
