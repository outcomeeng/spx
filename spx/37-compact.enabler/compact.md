...
- Given a worktree-session scope and session token, when the compact stash path is composed, then it resolves to `.spx/worktree/{session-token}/compact/stash.jsonl` under the local worktree root ([test](tests/compact.scenario.l1.test.ts))
