...
Replace direct withTestEnv usage only where the assertion needs a current spec-tree shape. Keep withTestEnv for primitive config and file tests that only need a temp productDir. T1 may edit shared helpers under `testing/harnesses/spec-tree/` and fixture generator infrastructure when those helpers are owned by this node; migration of consumer tests outside this node belongs to the owning packet unless a focused acceptance test is needed here. Prove filesystem and in-memory fixture paths come from the same fixture model. Open one PR and ask reviewers to audit fixture ownership, real-file coverage, and productDir vocabulary.
```
