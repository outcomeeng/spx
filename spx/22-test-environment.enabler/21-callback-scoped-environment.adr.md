...
This decision governs the API shape of the spec-tree test environment in `testing/harnesses/spec-tree/`. The enabler's assertions specify observable behavior; this ADR specifies the shape that makes those assertions true and the reasoning behind the inversion-of-control pattern.
...
Inversion of control pairs with a rich env object. Tests destructure the helpers they need; the env offers a uniform surface regardless of which helpers a specific test uses. The `withTestEnv` shape is an established pattern across related codebases, carrying familiarity for readers moving between products.
...
- **Separate cleanup-owning `withConfigEnv` and `withTreeEnv` entry points.** Rejected because the lifecycle is identical. The accepted split keeps one cleanup primitive and layers `withSpecTreeEnv` on top for current spec-tree fixture shape.
...
Files under `testing/harnesses/spec-tree/` contain `withTestEnv`, `withSpecTreeEnv`, their env types, helper implementations, and generators. Tests across the codebase call `withTestEnv(config, async ({ ... }) => { ... })` for primitive temp product directory needs and `withSpecTreeEnv(config, async ({ ... }) => { ... })` for current spec-tree-shaped fixtures — no `createHarness().cleanup()` pattern, no manual `mkdtemp`/`rm` in test bodies, no class instantiation with disposable methods.

### MUST

- `withTestEnv(config, callback)` is the sole cleanup-owning primitive for spec-tree test environments; `withSpecTreeEnv(config, callback, options?)` wraps it for current spec-tree-shaped fixtures ([review])
...
- Accept a product directory that falls outside `os.tmpdir()` — any root path is validated before use ([review])
