...
- Given an asynchronous property callback that does not resolve within the classification timeout, when the harness runs, then fast-check terminates the case and the harness throws a structured property failure ([test](tests/property-harness.scenario.l1.test.ts))
...
- Given `SPX_PROPERTY_SEED` holds an integer, when the harness runs, then the run uses that seed, so the same failing run reproduces ([test](tests/property-harness.scenario.l1.test.ts))
...
- Classification level maps to the harness-owned per-run timeout, one timeout per execution level ([test](tests/property-harness.mapping.l1.test.ts))
