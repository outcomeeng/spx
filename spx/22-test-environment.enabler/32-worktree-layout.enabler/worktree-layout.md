...
- Given a `GIT_*` variable set in `process.env`, when the harness runs the callback, then the variable is stripped for the callback's duration and restored to its prior value after the callback completes ([test](tests/worktree-layout.scenario.l1.test.ts))
