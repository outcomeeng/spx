...
describe("the overall verdict folds the per-check buckets by the fixed precedence broken > unknown > degraded > healthy", () => {
  ...
  it.each([
    ...
    {
      buckets: [VERDICT_BUCKET.UNKNOWN, VERDICT_BUCKET.DEGRADED, VERDICT_BUCKET.HEALTHY, VERDICT_BUCKET.NOT_APPLICABLE],
  ...
  ])("a bucket set whose most severe decisive bucket is the head folds to $overall", ({ buckets, overall }) => {
