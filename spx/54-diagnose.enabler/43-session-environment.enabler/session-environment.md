...
- The session-environment reading derives the current worktree's claimed state from the shared worktree pool snapshot plus any live claim named by `SPX_WORKTREE_CLAIM_PATH`, then classifies the derived reading with the same verdict and bucket mapping ([test](tests/session-environment-snapshot.mapping.l1.test.ts), [test](tests/session-environment-probe.mapping.l1.test.ts))
