...
`spx diagnose` represents worktree topology and occupancy for worktree-touching checks as one in-process worktree pool snapshot. The snapshot gatherer reads git facts and occupancy claims through injected dependencies, and the `worktree-pool`, `session-environment`, and `session-store` checks derive their readings from that shared snapshot rather than invoking `spx worktree status`.

## Rationale

The diagnose engine in `spx/54-diagnose.enabler/13-diagnose-engine.adr.md` keeps classification deterministic by separating reading gather from pure verdict functions. Worktree-touching checks share the same reality boundary: git topology from `spx/15-worktree-management.pdr.md`, shared occupancy claims from `spx/38-worktree.enabler/21-occupancy-claim.adr.md`, and shared `.spx/worktrees/` state from `spx/17-state.adr.md`. Gathering that reality once prevents divergent observations across checks, avoids recursive CLI process storms, and gives tests a single contract for generated git facts, claim bytes, and process-liveness observations.
...
- ALWAYS: the snapshot gatherer receives git facts, occupancy storage, process table, current-worktree identity, agent-session environment, and doing-session inputs through explicit dependency parameters, with production defaults wired at the command boundary ([audit])
