...
- Each supported local agent cache maps an installed configured methodology source to the observed version ([test](tests/methodology-cache.mapping.l1.test.ts))

### Properties

- Across installed numeric dotted and non-version cache directories, version selection uses a configured exact version when present, otherwise uses the highest numeric dotted installed version, and ignores non-version names unless one is the configured exact version ([test](tests/methodology-version-selection.property.l1.test.ts))
