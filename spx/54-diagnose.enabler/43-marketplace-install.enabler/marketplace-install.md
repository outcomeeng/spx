...
- A plugin, plugin cache, or marketplace clone found under the repository — a coding agent's project-scoped install such as `.codex/` or `.claude/` holding plugin content — is reported as a broken-bucket defect naming the path and the agent home spx sets for that coding agent, per `spx/12-agent-harness.pdr.md`
