...
PROVIDES the marketplace-install diagnose behavior — classifies the methodology marketplace's install state across the distinct plugin surfaces the registered agents declare, expected-against-installed, pairing the verdict with a remediation hint for both the whole-product diagnose report and the domain-owned marketplace-install diagnostic provider
