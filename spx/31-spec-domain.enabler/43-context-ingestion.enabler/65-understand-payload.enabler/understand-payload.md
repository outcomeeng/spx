...
- ALWAYS: where the shipped tree's `source.json` records `provides`, the payload fails naming both declarations when that value differs from the declared `methodology.version`, and fails naming the migration source when a declared `methodology.migratingFrom` falls outside the recorded `supports` or the record declares no range to hold it ([test](tests/understand-payload.compliance.l1.test.ts))
