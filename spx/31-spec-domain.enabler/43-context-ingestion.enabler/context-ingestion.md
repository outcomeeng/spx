...
- ALWAYS: context manifests include configured methodology identity, product spec, ancestor specs, applicable decisions, lower-index sibling specs, co-located evidence links, and every tracked product-level or on-path node `PLAN.md` and `ISSUES.md` file ([test](tests/context-ingestion.compliance.l1.test.ts))
