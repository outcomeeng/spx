...
- Given a read-class document contains a citation-shaped path carrying a relative path segment or continuing past the decision suffix, when the manifest is built, then the path binds no read entry, reaches no filesystem probe, and the command succeeds ([test](tests/cited-decisions.scenario.l1.test.ts))
