...
- A decision citation is a Markdown inline link whose href begins `spx/` and ends `.adr.md` or `.pdr.md`; bare path text, other link destinations, coordination notes, and undisplayed source content contribute no citation. ([test](tests/cited-decisions.compliance.l1.test.ts))
- A cited decision already selected structurally appears once, and repeated citations and cycles add no duplicate. ([test](tests/cited-decisions.compliance.l1.test.ts))
- A citation that resolves to no tracked decision fails the whole projection naming both the cited path and the citing document. ([test](tests/cited-decisions.compliance.l1.test.ts))
