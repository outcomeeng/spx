...
- `spx spec context show [targets...]` emits the context library's targetless or targeted document projection and accepts `--methodology`, `--coding-agent <name>`, `--loaded-product`, repeatable `--loaded-target <path>`, and `--loaded-methodology` under their declared compatibility rules. ([test](tests/spec-context-commands.scenario.l1.test.ts))
