...
- NEVER: `spx spec status` without `--update` executes node tests — only `--update` runs tests, per `spx/31-spec-domain.enabler/21-node-status.enabler/15-status-file-contract.pdr.md` ([review])
