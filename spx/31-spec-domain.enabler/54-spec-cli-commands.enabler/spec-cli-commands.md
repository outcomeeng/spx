...
- ALWAYS: command handlers operate on tracked `spx/` files using worktree-local root resolution per `spx/15-worktree-management.pdr.md` ([review])
...
- ALWAYS: `spx spec status --update` classifies a node passing only when its per-node run executed every one of the node's discovered test paths and passed; a run leaving any path unexecuted — a gated-out or unmatched language — does not pass, per `spx/31-spec-domain.enabler/54-spec-cli-commands.enabler/21-status-testing-delegation.adr.md` ([test](tests/spec-cli-commands.scenario.l1.test.ts))
