...
- Given a tracked `spx/` tree contains no current spec-tree nodes, when `spx spec status` or `spx spec next` runs, then the command returns deterministic empty-tree output ([test](tests/spec-cli-commands.scenario.l1.test.ts))
