...
3. Retire the per-node-run composition from the `--update` descriptor once no caller invokes it.
4. Re-derive every committed `spx.status.json` through the projector on fresh `dist` and confirm the diff carries no unintended node.
