...
- Given the packaged executable runs in a temp product directory with a current `spx/` tree, when `spx spec context show <target> --json` is invoked through the process boundary with uniquely abbreviated node-directory segments and a trailing separator, then it exits successfully and renders deterministic context under the canonical target node path ([test](tests/context-target.scenario.l2.test.ts))
- Given the packaged executable runs in a temp product directory with a current `spx/` tree, when `spx spec context show <target> --json --content` is invoked through the process boundary, then it exits successfully and every read entry carries content, digest, and byte count ([test](tests/spec-cli-contract.scenario.l2.test.ts))
- Given `spx spec context show <target> --content` is invoked without the machine output flag through the process boundary, when the command runs, then it exits non-zero with a deterministic diagnostic ([test](tests/spec-cli-contract.scenario.l2.test.ts))
