...
- Given the development CLI entry point runs in a temp product directory with a current `spx/` tree, when `spx spec context <target> --json` is invoked through the process boundary with uniquely abbreviated node-directory segments and a trailing separator, then it exits successfully and renders deterministic context under the canonical target node path ([test](tests/spec-cli-contract.scenario.l2.test.ts))
