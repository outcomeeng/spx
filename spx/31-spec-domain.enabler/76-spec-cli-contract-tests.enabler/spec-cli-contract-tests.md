...
- The packaged executable exposes targetless and targeted `spx spec context show` in text and JSON, preserving the selected document and reference entries across representations. ([test](tests/context-target.scenario.l2.test.ts))
- The packaged executable exposes `spx spec context list <targets...>` as the structural manifest command and rejects `--content` on `show`. ([test](tests/spec-cli-contract.scenario.l2.test.ts))
- The packaged executable accepts caller-declared loaded product, target, and methodology context, suppresses only entries covered at a sufficient projection mode, and rejects incompatible methodology flags. ([test](tests/spec-cli-contract.scenario.l2.test.ts))
