...
- ALWAYS: `spx spec status --update` derives pass/fail/not-run outcomes only for linked verification references and obtains those outcomes from the evidence the owning verification surface recorded; `declared` and `specified` nodes classify structurally without a run ([test](tests/node-status.compliance.l1.test.ts))
- NEVER: `spx spec status --update` replaces a reference's committed outcome when no recorded evidence resolves that reference — the committed outcome stands, so a fold neither forges an outcome no run produced nor discards a claim a run still supports ([test](tests/node-status.compliance.l1.test.ts))
