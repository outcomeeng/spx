...
Archive validates the result field declared by [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) before moving any session to `archive/`.
...
- ALWAYS: `spx session archive` reads the session's `result` field through `SESSION_FRONT_MATTER.RESULT` and rejects when the field is missing or empty per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-retention.compliance.l1.test.ts))
- NEVER: `spx session archive` moves a file whose `result` field is empty or absent — incomplete sessions are not added to the archived log per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-retention.compliance.l1.test.ts))
