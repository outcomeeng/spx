...
**Evidence:** [`session-cli.md`](session-cli.md)'s opening statement and its non-git ALWAYS rule now agree that the non-git refusal surfaces no diagnostic; the checklist rules are scoped to the linked-worktree refusal and do not govern the non-git case. The non-git ALWAYS rule's companion NEVER rule describes the non-git-repo diagnostic as reading correctly for "read, claim, retention, and create subcommands alike", which still needs confirming against `handoff` (a create subcommand) writing nothing.

**Resolution:** In the handoff-base implementation PR, render the non-git refusal as a silent non-zero exit with no stderr output, distinct from the linked-worktree checklist, and confirm the non-git-repo diagnostic's subcommand coverage in the non-git NEVER rule is consistent with `handoff` surfacing no diagnostic.
