...
- Given a session whose `result` is empty, when `spx session archive <id>` is executed through `node bin/spx.js`, then no file is moved, a diagnostic line naming `SessionInvalidResultError` is written to stderr, and the process exits non-zero ([test](tests/session-cli.compliance.l2.test.ts))
