...
- Given a JSON header that omits `goal`, when `spx session handoff` is executed through `node bin/spx.js`, then no file is written, a diagnostic line naming `SessionInvalidGoalError` is written to stderr, and the process exits non-zero ([test](tests/session-cli.scenario.l2.test.ts))
- Given stdin opening with the YAML-frontmatter delimiter `---`, when `spx session handoff` is executed through `node bin/spx.js`, then no file is written, a diagnostic line naming `SessionLegacyFrontmatterInputError` is written to stderr, and the process exits non-zero per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-cli.scenario.l2.test.ts))
