...
Frontmatter-validation errors raised by the underlying commands (`SessionInvalidContentError`, `SessionInvalidGoalError`, `SessionInvalidNextStepError`, `SessionInvalidResultError`) per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) propagate through the binding as non-zero exits with a diagnostic line on stderr.
...
- Given piped content that omits `goal`, when `spx session handoff` is executed through `node bin/spx.js`, then no file is written, a diagnostic line naming `SessionInvalidGoalError` is written to stderr, and the process exits non-zero ([test](tests/session-cli.scenario.l2.test.ts))
- Given a session whose `result` is empty, when `spx session archive <id>` is executed through `node bin/spx.js`, then no file is moved, a diagnostic line naming `SessionInvalidResultError` is written to stderr, and the process exits non-zero ([test](tests/session-cli.scenario.l2.test.ts))
- Given HEAD is detached, when `spx session handoff` is executed through `node bin/spx.js`, then no file is written, a diagnostic line naming `SessionDetachedHeadError` is written to stderr, and the process exits non-zero per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-cli.scenario.l2.test.ts))
...
- ALWAYS: diagnostic lines for `SessionInvalidContentError`, `SessionInvalidGoalError`, `SessionInvalidNextStepError`, `SessionInvalidResultError`, and `SessionDetachedHeadError` per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) include the error name and the offending session ID (when applicable) ([test](tests/session-cli.compliance.l2.test.ts))
