...
- Given HEAD is detached in the main checkout, when `spx session handoff` is executed through `node bin/spx.js`, then no file is written, a diagnostic line naming `SessionDetachedHeadError` is written to stderr, and the process exits non-zero per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-cli.compliance.l2.test.ts))
- Given a linked worktree whose working tree is dirty or whose HEAD is not detached at the default branch's tip, when `spx session handoff` is executed through `node bin/spx.js`, then no file is written, a diagnostic line naming `SessionWorktreeBaseError` is written to stderr, and the process exits non-zero per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-cli.compliance.l2.test.ts))
...
- ALWAYS: diagnostic lines for `SessionInvalidContentError`, `SessionInvalidGoalError`, `SessionInvalidNextStepError`, `SessionDetachedHeadError`, `SessionWorktreeBaseError`, `SessionLegacyFrontmatterInputError`, and `SessionInvalidJsonHeaderError` per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) include the error name and the offending session ID (when applicable) ([test](tests/session-cli.compliance.l2.test.ts))
