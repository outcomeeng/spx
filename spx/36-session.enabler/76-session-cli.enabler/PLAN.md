...
- Relocate the remaining descriptor to `src/interfaces/cli/{domain}.ts`: `audit` is the exception — it has no `src/commands/audit/` tier, so its reconciliation also creates that tier and extracts the handler logic (`runVerifyCommand`) out of `src/domains/audit/cli.ts`, leaving the domain layer pure.
