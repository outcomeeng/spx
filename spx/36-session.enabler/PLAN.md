...
- Author `testing/generators/session/` with at least:
  ...
  - `arbitraryNonStringArrayValue` (any YAML-representable value other than an array of strings — scalars, objects, arrays with non-string elements, mixed-type arrays — for the `specs`/`files` coercion property in auto-injection)
...
**Sequencing dependency:** the custom ESLint rule for `SESSION_FRONT_MATTER` literal enforcement (described in the Tests section above) must land before the test file `tests/session-store.compliance.l1.test.ts` can be re-authored to exercise the rule. The execution order is: (1) author the ESLint rule and its ADR, (2) re-author the compliance test to assert the rule fires on violating fixtures, (3) land the rest of the test re-author work, (4) implementation.
...
- `src/commands/session/handoff.ts` — drop the `buildSessionContent` default-substitution branch; validate non-empty `goal` and `next_step` from parsed YAML; reject empty content with `SessionInvalidContentError`; detect detached HEAD (when `git rev-parse --abbrev-ref HEAD` returns `HEAD`) and reject with `SessionDetachedHeadError`; otherwise prefill `branch` from `git rev-parse --abbrev-ref HEAD` and `worktree` from the helper introduced in `src/git/root.ts`; quote both `branch` and `worktree` through the `yaml` package's `stringify` before writing so values containing YAML-special characters round-trip cleanly
