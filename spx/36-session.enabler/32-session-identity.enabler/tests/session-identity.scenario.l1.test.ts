...
import {
  expectedSessionId,
  SESSION_IDENTITY_FIXTURE_YEAR,
  SESSION_IDENTITY_INVALID_TIMESTAMP,
  utcFixtureInstant,
} from "@testing/harnesses/session/session-identity";
...
describe("resolveAgentSessionId", () => {
  ...
  it("GIVEN path-unsafe Codex thread identity WHEN resolved THEN returns a safe token", () => {
    ...
    const resolved = resolveAgentSessionId({
      [AGENT_SESSION_ENV.CODEX_THREAD_ID]: pathUnsafeIdentity,
    });
...
describe("parseSessionId", () => {
  ...
  it("GIVEN invalid format WHEN parsed THEN returns null", () => {
    expect(parseSessionId(SESSION_IDENTITY_INVALID_TIMESTAMP)).toBeNull();
    expect(parseSessionId("")).toBeNull();
    expect(parseSessionId(`${SESSION_IDENTITY_FIXTURE_YEAR}/01/13 08:01:05`)).toBeNull();
  });

  it("GIVEN out-of-range month WHEN parsed THEN returns null", () => {
    expect(parseSessionId(`${SESSION_IDENTITY_FIXTURE_YEAR}-13-01${SESSION_ID_SEPARATOR}00-00-00`)).toBeNull();
    expect(parseSessionId(`${SESSION_IDENTITY_FIXTURE_YEAR}-00-01${SESSION_ID_SEPARATOR}00-00-00`)).toBeNull();
  });

  it("GIVEN out-of-range hour WHEN parsed THEN returns null", () => {
    expect(parseSessionId(`${SESSION_IDENTITY_FIXTURE_YEAR}-01-01${SESSION_ID_SEPARATOR}24-00-00`)).toBeNull();
  });

  it("GIVEN out-of-range minute WHEN parsed THEN returns null", () => {
    expect(parseSessionId(`${SESSION_IDENTITY_FIXTURE_YEAR}-01-01${SESSION_ID_SEPARATOR}00-60-00`)).toBeNull();
  });

  it("GIVEN out-of-range second WHEN parsed THEN returns null", () => {
    expect(parseSessionId(`${SESSION_IDENTITY_FIXTURE_YEAR}-01-01${SESSION_ID_SEPARATOR}00-00-60`)).toBeNull();
  });
});
