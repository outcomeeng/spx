...
- Agent-session identity resolution maps the source-owned sequence of declared agent identity variables, ordered most-specific-marker-first, to the first non-empty value it finds, and maps an environment where every declared variable is absent or empty to no identity ([test](tests/session-identity.mapping.l1.test.ts))
