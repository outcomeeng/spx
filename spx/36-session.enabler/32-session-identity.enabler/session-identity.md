...
- Given YAML front matter that contains a `result` key (sessions written under the previous frontmatter shape), when metadata is parsed, then the returned object does not include a `result` key and no error or warning is raised — the `result` field is silently dropped per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-identity.scenario.l1.test.ts))
- Given YAML front matter that contains a `worktree` key (sessions written under the previous frontmatter shape), when metadata is parsed, then the returned object does not include a `worktree` key and no error or warning is raised — the `worktree` field is silently dropped per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-identity.scenario.l1.test.ts))
...
- NEVER: `parseSessionMetadata` returns a `result` key — the frontmatter shape excludes `result` per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-identity.compliance.l1.test.ts))
- NEVER: `parseSessionMetadata` returns a `worktree` key — the frontmatter shape excludes `worktree` per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-identity.compliance.l1.test.ts))
