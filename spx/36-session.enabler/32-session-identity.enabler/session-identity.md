...
- ALWAYS: `parseSessionMetadata` returns the eight default-valued fields declared by [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) (`priority`, `specs`, `files`, `branch`, `worktree`, `goal`, `next_step`, `result`) as required keys with the PDR-declared defaults; `agent_session_id` and `created_at` are optional in the return type, matching the PDR-11 lifecycle column where each is recorded as absent when the corresponding source is unavailable ([review])
