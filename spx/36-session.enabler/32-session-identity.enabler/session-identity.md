...
- Given a session ID string in valid format, when parsed, then a Date object with matching UTC components is returned ([test](tests/session-identity.scenario.l1.test.ts))
...
- Given every agent-session environment identity, when agent-session identity is resolved, then `CODEX_THREAD_ID` is returned; given only `CLAUDE_CODE_SESSION_ID`, then it is returned; given only `SPX_AGENT_SESSION_ID`, then it is returned; given an empty `CODEX_THREAD_ID` with a non-empty `CLAUDE_CODE_SESSION_ID`, then the Claude Code identity is returned; given empty Codex and Claude Code identities with a non-empty `SPX_AGENT_SESSION_ID`, then the SPX-resolved identity is returned; given none or only empty identities, then no identity is returned ([test](tests/session-identity.scenario.l1.test.ts))
...
- NEVER: `parseSessionMetadata` returns a key outside the declared shape — only `priority`, `specs`, `files`, `git_ref`, `goal`, `next_step`, and (when the input carries them) `agent_session_id` and `created_at` appear in the parsed result per [`spx/36-session.enabler/11-session-frontmatter.pdr.md`](../11-session-frontmatter.pdr.md) ([test](tests/session-identity.compliance.l1.test.ts))
