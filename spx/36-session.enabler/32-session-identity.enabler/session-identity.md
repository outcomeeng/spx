...
- Given every agent-session environment identity, when agent-session identity is resolved, then `PI_SESSION_ID` is returned; given only `CODEX_THREAD_ID`, then it is returned; given only `CLAUDE_CODE_SESSION_ID`, then it is returned; given an empty `PI_SESSION_ID` with a non-empty `CODEX_THREAD_ID`, then the Codex identity is returned; given empty Pi and Codex identities with a non-empty `CLAUDE_CODE_SESSION_ID`, then the Claude Code identity is returned; given none or only empty identities, then no identity is returned ([test](tests/session-identity.scenario.l1.test.ts))
