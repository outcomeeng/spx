...
- Each unmet line in the handoff refusal checklist states its failure reason and a remedy; the unclean-working-tree remedy is to commit the changes or to run handoff from the root worktree ([review])
...
- The handoff refusal diagnostic renders an unresolved placeholder, such as the literal `origin/<default>`, in place of a concrete git value — every line substitutes the resolved git value ([review])
