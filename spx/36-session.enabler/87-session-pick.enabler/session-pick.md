...
- Given a resolved launch command, when the launcher hands off to the agent, then it suspends the parent's foreground-signal handling, spawns the agent, and on the agent's exit restores that handling and resolves the agent's status — a non-zero status when the agent exits without one or its binary cannot be spawned — so the descriptor always exits with a defined status rather than hanging on an unresolved promise or crashing on an unhandled spawn error ([test](tests/launch-agent.scenario.l1.test.ts))
...
- For every runtime, auto-continue flag, and session id, the launch command is the runtime's binary with a single prompt argument `<prefix>pickup <id>` — prefix `/` for claude, `$` for codex — extended by `--auto-continue` exactly when the flag is set ([test](tests/picker-model.property.l1.test.ts))
