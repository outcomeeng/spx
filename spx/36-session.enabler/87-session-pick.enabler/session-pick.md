...
- Given claimable sessions, when the picker mounts, then the frame lists each session with the highest-priority newest session selected and its goal and next step shown in the preview pane ([test](tests/picker-render.scenario.l2.test.ts))
- Given a mounted picker, when filter text is typed, then the frame narrows to the matching sessions and the preview follows the new selection ([test](tests/picker-render.scenario.l2.test.ts))
- Given a mounted picker over at least two sessions, when the down arrow then Enter is pressed, then the picker claims the second session, emits its `<PICKUP_ID>` tag to stdout, and unmounts ([test](tests/picker-render.scenario.l2.test.ts))
- Given a mounted picker, when Esc is pressed, then the picker unmounts with no session claimed, no `<PICKUP_ID>` tag emitted, and the queue unchanged ([test](tests/picker-render.scenario.l2.test.ts))
- Given an empty claimable queue, when the picker mounts, then the frame shows the empty-state message and Enter claims nothing ([test](tests/picker-render.scenario.l2.test.ts))
...
- ALWAYS: the picker restores the terminal — exits raw mode and leaves the alternate screen — on unmount whether reached by claim, cancel, or a forwarded SIGINT or SIGTERM, per [`spx/13-cli.enabler/21-terminal-ui.adr.md`](../../13-cli.enabler/21-terminal-ui.adr.md) ([audit])
