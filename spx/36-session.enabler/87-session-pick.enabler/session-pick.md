...
- ALWAYS: on a launch keystroke the picker restores the terminal and the process is then replaced by the resolved agent command, so claude or codex inherits the terminal; the replacement is performed through an injected runner, not a direct process API in the descriptor, per [`spx/14-cli-composition.adr.md`](../../14-cli-composition.adr.md) ([audit])
