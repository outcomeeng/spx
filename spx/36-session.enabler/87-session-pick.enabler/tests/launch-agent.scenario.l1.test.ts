 ...
 * makes emit `exit` or `error`. The launch command and the exit statuses are
...
import { RecordingLaunchRunner } from "@testing/harnesses/session/launch-runner";
...
describe("launchAgent exit status", () => {
  it("resolves with the status the agent process exits with", async () => {
    const status = sample(fc.integer({ min: 0, max: 255 }));
    const runner = new RecordingLaunchRunner();

    const pending = launchAgent(runner, sampleCommand());
    runner.children[0].emitExit(status);

    expect(await pending).toBe(status);
  });

  it("resolves a non-zero status when the agent exits without one", async () => {
    const runner = new RecordingLaunchRunner();
  ...
  it("resolves a non-zero status when the agent binary cannot be spawned", async () => {
    const runner = new RecordingLaunchRunner();
