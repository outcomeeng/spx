...
PROVIDES the Python validation pipeline — stages for lint, type checking, and AST enforcement
...
- Python stages: lint, type-check, ast-enforcement — each stage is provided by a leaf enabler child ([test](tests/python-validation.integration.test.ts))

### Compliance

- ALWAYS: every Python stage is gated on `detectPython` reporting present ([test](tests/python-validation.integration.test.ts))
- NEVER: invoke a Python stage against a project where language detection reports Python absent ([test](tests/python-validation.integration.test.ts))
