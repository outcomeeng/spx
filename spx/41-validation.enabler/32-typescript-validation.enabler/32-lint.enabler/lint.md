...
- ALWAYS: repository lint-debt manifest checks run inside `spx validation lint`, are skipped for projects without those manifests, and never run while loading `eslint.config.ts` ([test](tests/lint-policy.compliance.l1.test.ts))
