...
- Whitelisted positions produce no diagnostic for either registry rule: the string argument to `describe`/`it`/`test`/`describe.each`/`it.each`/`test.each`, single-member literal type positions, object-key positions, and any string that is a substring of a larger identifier. Bare two-or-more-member string unions remain violations through `no-bare-string-unions`. ([test](tests/no-hardcoded-spec-tree-node-states.mapping.l1.test.ts), [test](tests/no-hardcoded-spec-tree-node-kinds.mapping.l1.test.ts))
...
- ALWAYS: broad test-constant migration uses a shrink-only debt manifest; manifest-covered nodes downgrade only `spx/no-test-owned-domain-constants` to warnings ([test](tests/eslint-rules.scenario.l2.test.ts))
- ALWAYS: new test-owned-constant debt manifest entries fail config validation unless the committed baseline already contains them ([review])
