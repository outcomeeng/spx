...
- A terminal-write call — a `writeStdout`/`writeStderr` adapter method, `process.stdout.write`, `process.stderr.write`, or a `console` output method — whose written argument is a template literal carrying an interpolation or a concatenation with a non-literal operand maps to lint error; composing that argument through the terminal-text primitive maps to no diagnostic ([test](tests/no-unescaped-terminal-text.mapping.l1.test.ts), [enforce](../../../../eslint-rules/no-unescaped-terminal-text.ts))
- Whitelisted positions produce no diagnostic for `no-unescaped-terminal-text`: a written argument that is a plain literal, a template literal with no interpolation, an identifier or member expression carrying no embedding, a call whose result the terminal-text primitive rendered, and any write inside the terminal-text module itself ([test](tests/no-unescaped-terminal-text.mapping.l1.test.ts))
