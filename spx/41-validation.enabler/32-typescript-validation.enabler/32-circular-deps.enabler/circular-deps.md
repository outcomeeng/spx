...
- Given a TypeScript project with a circular dependency outside the production TypeScript scope, when `spx validation circular --scope production` runs, then the command exits zero and reports no circular dependencies ([test](tests/circular-deps.scenario.l2.test.ts))
