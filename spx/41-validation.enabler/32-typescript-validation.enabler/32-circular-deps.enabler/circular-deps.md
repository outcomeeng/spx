...
- Given `spx validation circular <path>` runs, when circular dependency validation invokes dependency-cruiser, then dependency-cruiser receives only the project-relative requested path after validation path filtering ([test](tests/circular-deps.scenario.l1.test.ts))
