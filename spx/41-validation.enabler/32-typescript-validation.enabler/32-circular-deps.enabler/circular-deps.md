...
- Given a TypeScript project with a circular dependency, when `spx validation circular` runs, then the command exits non-zero and reports the cycle ([test](tests/circular-deps.scenario.l1.test.ts))
- Given a TypeScript project with a circular dependency outside the production TypeScript scope, when `spx validation circular --scope production` runs, then the command exits zero and reports no circular dependencies ([test](tests/circular-deps.scenario.l1.test.ts))
- Given a TypeScript project with a cycle composed only of erased type-only imports, when `spx validation circular` runs, then the command exits zero and reports no circular dependencies ([test](tests/circular-deps.scenario.l1.test.ts))
...
- Given TypeScript scope directories contain JavaScript files beside TypeScript files, when circular dependency validation invokes dependency-cruiser, then dependency-cruiser receives TypeScript source globs and TypeScript-only extension filters ([test](tests/circular-deps.scenario.l1.test.ts))
- Given dependency-cruiser returns non-structured reporter output, when circular dependency validation evaluates the result, then validation fails with a clear dependency-cruiser reporter error ([test](tests/circular-deps.scenario.l1.test.ts))
- Given `spx validation circular --files <path>` runs, when circular dependency validation invokes dependency-cruiser, then dependency-cruiser receives only the project-relative requested path after validation path filtering ([test](tests/circular-deps.scenario.l1.test.ts))
- Given a project where language detection reports TypeScript absent, when `spx validation circular` runs, then the command emits the TypeScript-absent skip result and dependency-cruiser does not run ([test](tests/circular-deps.scenario.l1.test.ts))
- Given a project with TypeScript files and no `tsconfig.json`, when `spx validation circular` runs, then the command emits the TypeScript-absent skip result and dependency-cruiser does not run ([test](tests/circular-deps.scenario.l1.test.ts))
