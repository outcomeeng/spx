...
Source-owned imports across the test boundary eliminate the divergence class. A test importing the source value cannot diverge from it. The same property extends to test-invented constants recurring across files: a single shared declaration site collapses the rename surface to one edit. Inline literals carrying domain meaning are prohibited because they have no single declaration site — restricting raw literals in test bodies forces the author to classify each value as source-owned (import from source) or test-invented and recurring (import from shared test support).
...
| Test-invented constants live in shared test support rather than co-located test files              | Shared test support is the canonical home; co-located constants have no single declaration site once duplicated                                               |
...
- A literal value that recurs across two or more test files is either imported from source or imported from shared test support.
...
- Test-invented values that recur across two or more test files are declared once in shared test support with an explicit semantic name and imported by each test that uses them ([review])
- Raw numeric literals outside the structural set (`-1`, `0`, `1`, `2`) are imported from their owning module: product-owned values from `src/`, protocol values from package constants, test-infrastructure values from shared test support ([review])
...
- Place a test-invented constant inline in a test file when the same constant recurs in another test file ([review])
