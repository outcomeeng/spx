...
This decision governs how the `--allowlist-existing` helper persists allowlist entries to `spx.config.*` at the product directory — which file is written, how the original syntax format is preserved, write atomicity, the file-creation path when no config exists, exit-code semantics, and dependency injection of filesystem reader and writer.
...
When no `spx.config.*` exists at the product directory, the helper creates `spx.config.yaml`. YAML is the ergonomic default for human-edited configs (comments, hierarchical structure, no quote noise) and matches the format spx uses for its own product config. Writing JSON-by-default is defensible but produces a file with weaker affordances for the user's later maintenance.
...
- **Always write `spx.config.yaml` regardless of existing format.** Rejected because a product repository that committed to JSON or TOML would suddenly carry a second config file in a different format; the next `resolveConfig` invocation triggers the ambiguity error path.
...
| Atomic rename relies on temp file and target sharing a filesystem     | The helper writes the temp file alongside the target in the product directory; product directory and its sibling temp file always share a filesystem |
...
- Writes are atomic: the helper writes to a sibling temp file in the product directory, then renames over the destination — readers observe the old file or the new file, never a partial state ([review])
...
- Re-implement product-directory file detection inside the helper — divergence from `resolveConfig`'s detection produces unpredictable write targets ([review])
