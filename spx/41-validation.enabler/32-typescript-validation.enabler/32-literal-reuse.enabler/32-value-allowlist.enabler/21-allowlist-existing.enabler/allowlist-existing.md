...
PROVIDES the bulk-silence adoption helper — records every current literal-reuse finding's value into the project's `validation.literal.values.include` in one operation, deduplicating against existing entries and preserving the original config file format
...
- Given a project with one or more current findings, when the user runs `spx validation literal --allowlist-existing`, then every distinct value currently flagged is appended to `validation.literal.values.include` in the project's `spx.config.*` file (creating the file or `validation.literal.values` section if absent, deduplicating against existing entries, sorted alphabetically), and a subsequent `spx validation literal` run against the unchanged source reports zero findings ([test](tests/allowlist-existing.scenario.l1.test.ts))
...
- ALWAYS: `--allowlist-existing` writes only to `validation.literal.values.include` — never to `validation.literal.values.presets`, `validation.literal.values.exclude`, or any other section of `spx.config.*` ([test](tests/allowlist-existing.compliance.l1.test.ts))
