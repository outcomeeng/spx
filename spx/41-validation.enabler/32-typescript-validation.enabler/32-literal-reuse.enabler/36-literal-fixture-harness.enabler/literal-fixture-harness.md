...
- ALWAYS: the public API is callback-scoped — `withLiteralFixtureEnv(config, callback) → Promise<T>` exposes no caller-owned cleanup handle, matching the test-environment callback pattern ([audit])
...
- NEVER: use `vi.mock`, `jest.mock`, `memfs`, or any filesystem-mocking mechanism — fixtures are written to a real OS temp directory under `os.tmpdir()` ([audit])
