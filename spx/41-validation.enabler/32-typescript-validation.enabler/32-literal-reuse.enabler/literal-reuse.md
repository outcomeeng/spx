...
- Given a fixture-writer call contains another fixture-writer call as its payload, when literals are collected, then both writer argument lists are treated as setup data ([test](tests/literal.scenario.l1.test.ts))
...
- Given a test file destructures from an inline object expression, when literals are collected, then destructuring defaults and inline object values still contribute occurrences because no fixture object identifier names the data ([test](tests/literal.scenario.l1.test.ts))
