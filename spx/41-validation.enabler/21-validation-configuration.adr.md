...
- For every registered validation stage, full-pipeline participation equals the stage descriptor's default participation unless `spx validation all` receives that stage's invocation-local override, in which case participation equals the inverse of the default.
...
- ALWAYS: declare every stage's default full-pipeline participation and required invocation-local override in the stage descriptor, derive the override participation as the inverse of the default, and derive `spx validation all` option registration and run/skip decisions from that metadata ([audit])
- NEVER: declare an override participation independently from its stage default — the inverse relationship has one source of truth ([audit])
