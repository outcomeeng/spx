...
- Given validation-all help is requested, when `spx validation all --help` is invoked, then the help output lists exactly one full-pipeline override flag derived from every registered stage's participation metadata ([test](tests/dispatch.scenario.l2.test.ts))
- Given a validation stage's default participation is overridden for one full-pipeline run, when `spx validation all <override-flag>` is invoked, then that stage follows the inverse of its default, emits its configured skip output when the inverse is skip unless `--quiet` is set, and the other validation stages follow their defaults ([test](tests/dispatch.scenario.l2.test.ts))
...
- ALWAYS: register exactly one full-pipeline participation override flag per stage on `spx validation all` from stage descriptors and scope it to that full-pipeline invocation; standalone stage subcommands do not accept full-pipeline override flags ([test](tests/dispatch.compliance.l2.test.ts))
