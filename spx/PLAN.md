...
**Methodology vocabulary (`spx/11-methodology-vocabulary.pdr.md`).** The methodology PDR reserves `persistence`, `delivery`, `records`, `journals`, and `snapshots` and defines them (`persistence` = retained-artifact model — records / journals / snapshots; `delivery` = ephemeral projection to external surfaces), and defines `backend` (the implementation-axis term — an adapter implementing a persistence or delivery contract). `state`, `status`, and `materialization` stay reserved and unchanged — each is overloaded in the current tree (`state`: node lifecycle vs. stored execution state; `status`: node-status read-back — `spx/31-spec-domain.enabler/21-node-status.enabler` — vs. a backend label; `materialization`: the `spx/23-spec-tree.enabler/24-materialization.enabler` backend adapter vs. a generic produce-artifact action in `spx/34-verification.enabler/verification.md` and `spx/22-test-environment.enabler/32-spec-tree-fixtures.enabler`), so retuning them is deferred to the slices that reconcile those uses. Per that PDR's ALWAYS rule the product-wide decision changes before lower layers consume a new methodology term, so durable specs, decisions, tests, and code may now adopt these terms; delivery-order step 2 (below) is this reservation.
...
2. **Reserve the target methodology vocabulary** — `spx/11-methodology-vocabulary.pdr.md` reserves `persistence`, `delivery`, `records`, `journals`, and `snapshots`, defines them and `backend`, and leaves the overloaded terms `state`, `status`, and `materialization` reserved and unchanged (their retuning is deferred to the slices that reconcile their uses). This reservation precedes every durable consumer, because the changes-domain build next authors durable specs, decisions, and source that use `records`.
