...
**Methodology vocabulary (`spx/11-methodology-vocabulary.pdr.md`).** The methodology PDR reserves `persistence`, `delivery`, `records`, `journals`, and `snapshots`: it defines `persistence` (retained-artifact model — records / journals / snapshots) and `delivery` (ephemeral projection to external surfaces), retunes `state` (a node's evidence-derived lifecycle standing), `status` (a backend or projection label mapped into product semantics), and `backend` (the preferred implementation-axis term — an adapter implementing a persistence or delivery contract), and keeps `materialization` reserved as the legacy name for `backend` until its consumers migrate. Per that PDR's ALWAYS rule the product-wide decision changes before lower layers consume a new methodology term, so durable specs, decisions, tests, and code may now adopt these terms; delivery-order step 2 (below) is this reservation.
