...
2. **Reserve the target methodology vocabulary** — amend `spx/11-methodology-vocabulary.pdr.md` (via `/author` + `pdr-auditor`) to define `persistence`, `delivery`, `records`, `journals`, and `snapshots` and their relation to the already-reserved `backend`, `state`, `status`, and `materialization`. This is the first build action after this plan merges and precedes every durable consumer, because the changes-domain build next authors durable specs, decisions, and source that use `records`.
