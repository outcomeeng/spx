...
**Methodology vocabulary (`spx/11-methodology-vocabulary.pdr.md` — reserve first, before any durable consumer).** This plan introduces `persistence`, `delivery`, `records`, `journals`, and `snapshots` as *proposed* target vocabulary; the methodology PDR reserves none of them yet. That PDR's own ALWAYS rule requires the product-wide decision to change before lower layers consume a new methodology term, so reserving these terms is the **first foundation-step action**: amend the PDR via `/author` + `pdr-auditor` to define `persistence` (retained-artifact model; records / journals / snapshots), `delivery` (ephemeral projection to external surfaces), and their relation to the already-reserved `backend`, `state`, `status`, and `materialization`, before any durable spec, decision, test, or code adopts them. Retuned senses to reserve: `state` = node/evidence-derived product state; `status` = a backend or projection label mapped into product semantics; `backend` = an implementation of a persistence or delivery contract; `materialization` is removed or sharply narrowed. This coordination-note-only changeset proposes the vocabulary and sequences its reservation; it does not itself amend the PDR — that product-decision change is a foundation-step task outside this changeset's diff, tracked here.
...
**`spx verify` is the decided SPX home for verification projection and validation** — not an open product fork. `src/domains/verify/verify.ts` already holds the typed finding validation, disposition typing, input replay, and idempotency the plugin stop-gap awaits; the remaining lifecycle — `spx/34-verification.enabler/32-verify.enabler` (its cross-lifecycle verb mapping) and its child `43-terminal-projection.enabler` (finish / status / render) — is still `spx/EXCLUDE`-gated. The journal is the backend-neutral channel/substrate `spx verify` writes through. The plugin decisions already require this; the SPX side is simply unfinished.
...
3. **Persistence + verification foundation** (additive): first reserve the `persistence` / `delivery` / `records` / `journals` / `snapshots` vocabulary in `spx/11-methodology-vocabulary.pdr.md` (per the vocabulary note above); then name and lift `records` / `journals` / `snapshots` into a top-level persistence concern orthogonal to verification (rename `spx/18-state.enabler` → `spx/18-persistence.enabler`); finish `spx verify` (`spx/34-verification.enabler/32-verify.enabler/43-terminal-projection.enabler`) as the SPX projection/validation home; migrate the plugin skills off their Python stop-gap; then retire the journal's surface misplacement, move GitHub persistence backend semantics out of `spx/21-infrastructure.enabler/43-github-ci.enabler`, and split `spx/21-infrastructure.enabler/43-github-ci.enabler/21-snapshot-adapter.enabler` into its persistence and delivery halves.
...
| Question                                                                                                                                                  | Status                                                                                              |
| --------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| journal `list` / `read-set` — journal verbs, or `spx verify` inspection?                                                                                  | Open; resolve when the journal substrate is separated from the surface and from `spx verify`.       |
...
Resolved by this discussion (no longer open): the verify / journal boundary (SPX owns projection & validation via `spx verify`; journal is the substrate); the persistence-vs-backend axes and their names (`persistence` with records / journals / snapshots, `backend`, `delivery`; `state` reserved for volatile node lifecycle); `.surface` sequencing (deferred).
...
| `spx/34-verification.enabler/PLAN.md`                                                           | R    | verification = 5 types; `spx verify` is the SPX projection home                            |
