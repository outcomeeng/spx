...
This note records the restructuring of spx into clean, orthogonal layers: **surfaces** (how the product is consumed), **intermediate composition layer** (cross-library orchestration), **domain libraries**, **persistence** (durable storage — records, journals, snapshots), **delivery** (projection to external surfaces), and **backend** (environment boundaries providing persistence and delivery contracts). Product truth lives in `spx/spx.product.md`, decisions, specs, tests, and source; this file records the target architecture, the delivery order that reaches it, and an index of every descendant `PLAN.md`. It is not product truth, and it steers work only after each note is reconciled against the durable layers.
...
flowchart TB
  ...
  subgraph Composition["spx/31-spec-domain.enabler — intermediate composition layer (cross-library orchestration only)"]
  ...
  subgraph Backend["backend — environment boundaries providing persistence & delivery contracts"]
...
- `spx/17-state.adr.md`, `spx/15-worktree-management.pdr.md`, and the `spx/18-state.enabler` subtree — use state-named paths or storage-sense state prose that aligns when the persistence owner is built; the path rename target is `spx/18-persistence.enabler` (step 4).
...
- CLI-composition decisions and specs that cite `spx/14-cli-composition.adr.md` — use `command domain` for a source-layout unit across `src/domains/{domain}/`, `src/commands/{domain}/`, and `src/interfaces/cli/{domain}.ts`, and use `interface` in source-path names; align these as qualified source-layout compounds when CLI composition is reconciled with the methodology domain and interface vocabulary. First affected examples include `spx/14-cli-composition.adr.md`, `spx/15-agent-run-journal.enabler/32-journal-module-structure.adr.md`, `spx/54-diagnose.enabler/13-diagnose-engine.adr.md`, and `spx/60-surfaces.enabler/21-cli-surface.enabler/21-journal.enabler/13-journal-module-structure.adr.md`.
- Persistence decisions and specs that say `consumer` or `consumer domain` for a module or product area calling a persistence API — align these as API-consumer or persistence-consumer compounds when consumer vocabulary is reconciled. First affected examples include `spx/17-state.adr.md` and `spx/18-state.enabler/state.md`.
...
flowchart LR
  ...
  subgraph UC["intermediate composition layer (spec-domain)"]
...
The classification ("passing only when every linked reference passes; any unexecuted reference records `not-run`") spans spec-tree, verification, and test, so it belongs in no single library, and a future `mcp spec status` must reuse it, so it belongs in no CLI wrapper. That forces the intermediate composition layer `spx/31-spec-domain.enabler` between surfaces and libraries. A command with no cross-library orchestration (`spx config show`) skips the layer and its wrapper calls the library directly — the intermediate composition layer is per-need, not per-domain.
...
2. **Reserve the target methodology vocabulary, structure, and node kinds** — `spx/09-methodology-vocabulary.pdr.md` reserves and defines the full target vocabulary — `durable map`, `area`, `node type`, `dependency order`, `decision reach`, `assertion`, `evidence`, `state`, `status`, `persistence`, `records`, `journals`, `snapshots`, `delivery`, `backend`, `materialization`, `canonical suffix readiness`, `substrate`, `capability`, `enabler`, `domain`, `interface`, `consumer`, `surface`, and `outcome` — admitting qualified compounds; `spx/11-methodology-structure.pdr.md` reserves substrate, capabilities, domains, interfaces, and surfaces as structural areas whose area role is independent of the node suffix; `spx/10-methodology-node-kinds.pdr.md` reserves `.enabler` and `.outcome` as admitted node types, reserves `.substrate`, `.domain`, `.interface`, and `.surface` as role suffixes governed by canonical suffix readiness, treats `.capability` as historical grammar vocabulary rather than a future suffix, and assigns role ownership through specs and decisions rather than suffix-derived classification. Specs whose base-term usage predates the vocabulary align downstream (vocabulary-alignment list above). This reservation precedes every durable consumer, because the changes-domain build next authors durable specs, decisions, and source that use `records` and structural area ownership.
...
5. **Surface wrapper migration**: move the genuinely-thin CLI wrappers (config, session, validation, worktree) into `spx/60-surfaces.enabler/21-cli-surface.enabler/`, one reviewable slice each; split `spec-cli-commands` and `verify` so only the thin binding moves and the intermediate composition layer / library behavior stays.
...
| Question                                                                                                                                                  | Status                                                                                              |
| --------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Does `spx/31-spec-domain.enabler` keep its name or generalize to a product-wide intermediate composition layer?                                           | Open; `/decompose` + `/interview` after the persistence / verify split settles its remaining scope. |
...
| `spx/31-spec-domain.enabler/PLAN.md`                                                                                           | R    | intermediate composition layer; consumer boundary                                                     |
...
| `spx/34-verification.enabler/32-verify.enabler/PLAN.md`                                                                        | R    | verification-run lifecycle parent pointers for run-set and type expansion                             |
| `spx/34-verification.enabler/32-verify.enabler/54-run-set-orchestration.enabler/PLAN.md`                                       | R    | merge-period run-set envelope for repeated verification runs with expanding scope                     |
| `spx/34-verification.enabler/32-verify.enabler/65-review.enabler/PLAN.md`                                                      | R    | review verification type payload shape and plugin migration                                           |
| `spx/34-verification.enabler/32-verify.enabler/65-audit.enabler/PLAN.md`                                                       | R    | audit verification type payload shape, unit coverage, and terminal rollup                             |
