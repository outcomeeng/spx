...
- ALWAYS: `spx verification <type> run` roots discovery, the run selector, the recorder, and the runner at the worktree product root the invocation directory resolves to, per `spx/15-worktree-management.pdr.md`, from any directory inside the product, and at the invocation directory itself with a warning outside a repository ([test](tests/execute-run.compliance.l1.test.ts))
