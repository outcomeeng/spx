...
- ALWAYS: an operand that selects no discovered test file, or a verification type the executor resolves no runner for, is reported and opens no run, and a product directory that supplies no runner is reported by name with a non-zero exit after the run seals ([test](tests/execute-run.compliance.l1.test.ts))
