...
- ALWAYS: an operand that selects no discovered test file, or a verification type the executor resolves no runner for, is reported and opens no run, and a product directory that supplies no runner is reported by name with a non-zero exit after the run seals when no other language streamed ([test](tests/execute-run.compliance.l1.test.ts))
- ALWAYS: `spx verification <type> run` roots discovery, the run selector, and the runner at the local worktree root the invocation directory resolves to, from any directory inside the product, and invokes the recorder from that root so its branch-scoped run store resolves to the Git common-dir product root the worktrees share, per `spx/15-worktree-management.pdr.md`; outside a repository the invocation directory itself is the root and the result carries a warning ([test](tests/execute-run.compliance.l1.test.ts))
- ALWAYS: a run the command cannot complete — a recorder or runner failure — is reported as a diagnostic with a non-zero exit, never as an unhandled process failure ([test](tests/execute-run.compliance.l1.test.ts))
