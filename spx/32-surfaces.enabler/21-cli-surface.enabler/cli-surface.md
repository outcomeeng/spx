...
CAN invoke SPX capabilities through a consistent CLI contract while the command implementations compose lower-index interface-neutral libraries for product behavior
...
- ALWAYS: CLI command nodes compose lower-index product capability nodes for interface-neutral behavior instead of redefining storage, state, verification, or spec-tree semantics inside the surface ([audit])
