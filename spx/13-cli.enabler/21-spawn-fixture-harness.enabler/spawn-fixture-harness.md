...
PROVIDES a subprocess fixture that spawns a command, captures stderr, counts observed stdout bytes, can destroy the parent's stdout read end after a delay, and maps signal exits through the POSIX `128 + signal` convention
...
- ALWAYS: signal exits map through the source-owned POSIX signal-number table, returning `-1` only for an unknown signal ([audit])
