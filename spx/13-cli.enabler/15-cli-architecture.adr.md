...
| Managed subprocess call sites cannot customize `stdio` directly                                                                 | Descriptor containment is a lifecycle invariant; current validation tools read files rather than stdin, and call sites that need interactive stdin require a separate documented lifecycle mechanism |
