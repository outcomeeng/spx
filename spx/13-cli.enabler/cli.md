...
- Composition idempotence: for every value, composing already-composed terminal text yields the same rendered output as composing it once, so a value escaped upstream is never escaped twice ([test](tests/terminal-text.property.l1.test.ts))
