...
- External-segment safety: for every value composed as an external segment of terminal text, the rendered output holds no code point below `FIRST_PRINTABLE_CHAR_CODE` and never `DEL_CHAR_CODE` in that segment ([test](tests/terminal-text.property.l1.test.ts))
- Authored preservation: for every string composed as an authored segment, the rendered output reproduces it byte-for-byte, so product-owned styling and line structure survive composition ([test](tests/terminal-text.property.l1.test.ts))
- Composition transparency: for every already-composed segment, an enclosing composition reproduces it unchanged — an authored segment keeps its bytes and an escaped segment is never escaped twice ([test](tests/terminal-text.property.l1.test.ts))
- Display-bound external token: for every value composed as an external display token, the rendered segment holds no code point below `FIRST_PRINTABLE_CHAR_CODE` and never `DEL_CHAR_CODE`, is at most `MAX_CLI_ARGUMENT_DISPLAY_LENGTH` characters, and ends in `ELLIPSIS_TOKEN` when the escaped value exceeds that bound ([test](tests/terminal-text.property.l1.test.ts))
- Serialized-document safety: for every JSON-serializable value, the rendered JSON document holds no code point below `FIRST_PRINTABLE_CHAR_CODE` outside its own line structure and never `DEL_CHAR_CODE`, and parsing it yields the value unchanged ([test](tests/terminal-text.property.l1.test.ts))
...
- ALWAYS: the composed-text write accepts only text carrying the escaping decisions made where its values were embedded, while the pass-through write relays a document byte-for-byte, so a control byte inside a relayed document survives and the same byte embedded in composed output is escaped ([test](tests/pass-through-boundary.compliance.l1.test.ts))
- ALWAYS: a caller that redirects the composed-text write receives pass-through output at the same destination unless it redirects the relay separately, because both channels write one stream ([test](tests/pass-through-boundary.compliance.l1.test.ts))
- NEVER: the external-value decision accepts text already composed; a `TerminalText` handed to it resolves to a type no composition admits, so escaping composed text a second time fails to compile ([test](tests/terminal-text.compliance.l1.test.ts))
