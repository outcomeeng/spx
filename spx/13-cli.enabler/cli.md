...
- Output safety: for every input, every code point in the output is at or above the first printable byte and never DEL ([test](tests/sanitize.property.l1.test.ts))
...
- Printable preservation: for every non-empty input string whose length is at most `MAX_CLI_ARGUMENT_DISPLAY_LENGTH` and whose every code point is at or above the first printable byte and never DEL, the sanitizer returns the input unchanged ([test](tests/sanitize.property.l1.test.ts))
...
- External-segment safety: for every value composed as an external segment of terminal text, the rendered output holds no code point below the first printable byte and never DEL in that segment ([test](tests/terminal-text.property.l1.test.ts))
...
- Display-bound external token: for every value composed as an external display token, the rendered segment holds no code point below the first printable byte and never DEL, is at most `MAX_CLI_ARGUMENT_DISPLAY_LENGTH` characters, and ends in `ELLIPSIS_TOKEN` when the escaped value exceeds that bound ([test](tests/terminal-text.property.l1.test.ts))
- Serialized-document safety: for every JSON-serializable value, the rendered JSON document holds no code point below the first printable byte outside its own line structure and never DEL, and parsing it yields the value unchanged ([test](tests/terminal-text.property.l1.test.ts))
