...
- The factory's git invocations all happen at construction — the `git ls-files -z` scope query, plus `git rev-parse --git-common-dir` and `git config --type=path --get core.excludesFile` lookups when `--no-ignore-vcs` is active; query methods perform no filesystem or subprocess I/O.
...
- ALWAYS: the factory invokes `git ls-files -z` once at construction with the override-derived arguments, plus `git rev-parse --git-common-dir` and `git config --type=path --get core.excludesFile` lookups when `noIgnoreVcs` is active; query methods are pure over the constructed in-memory set ([audit])
