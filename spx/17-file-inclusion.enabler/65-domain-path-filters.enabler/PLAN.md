...
Wire typed config-backed domain path-filter inputs through the file-inclusion resolver so each validate, test, audit, review, evaluate, and internal traversal consumer narrows scope within the git-tracking default per `spx/17-file-inclusion.enabler/11-ignore-defaults.pdr.md`.
