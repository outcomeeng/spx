...
- ALWAYS: file inclusion receives typed path-filter values from callers and does not read `spx.config.*` directly ([test](tests/domain-path-filters.compliance.l1.test.ts))
