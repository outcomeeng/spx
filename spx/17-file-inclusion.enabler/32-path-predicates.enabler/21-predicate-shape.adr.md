...
- ALWAYS: the domain-path-filter predicate's matched decisions carry the matched include or exclude pattern in `detail`; the git-tracking predicate omits `detail` because git's included-set enumeration exposes membership rather than the responsible ignore source ([audit])
