...
- ALWAYS: the git-tracking predicate maps an included-set membership result to a non-match and a missing membership result to a matched exclusion decision ([mapping])
- ALWAYS: the domain-path-filter predicate maps include misses and exclude matches to matched exclusion decisions, and maps paths admitted by the filter to non-matches ([mapping])
- ALWAYS: repeated invocations of each predicate with equal path and equal state return equal `LayerDecision` values ([property])
- ALWAYS: predicate decisions are independent across layers: one predicate's result does not depend on another predicate's state or decision ([property])
...
- ALWAYS: the domain-path-filter predicate's matched decisions carry the matched exclude pattern, or the complete configured include-prefix set when no include pattern matches, in `detail`; the git-tracking predicate omits `detail` because git's included-set enumeration exposes membership rather than the responsible ignore source ([audit])
