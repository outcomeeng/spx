...
filter positional path operands through `validation.paths`, so an explicit caller path can be
...
explicit-path scope. `spx validation all ...` inherits the same behavior
for those stages. Observed on June 12, 2026:

```bash
tsx src/cli.ts validation lint spx/36-audit.enabler/tests/audit.scenario.l1.test.ts
```
...
```bash
tsx src/cli.ts validation lint spx/41-test.enabler/tests/test.scenario.l1.test.ts
```
...
`validation all` path-scoped invocations can skip files the caller intentionally
...
validation, update the validation ADR first, then route validation path operands
through a shared helper that normalizes explicit paths without applying domain
path filters to them, including the `validateLiteralReuse` / `applyPathFilter`
integration point.
