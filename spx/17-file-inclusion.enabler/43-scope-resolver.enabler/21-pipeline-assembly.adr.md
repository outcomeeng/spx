...
- Automatic walking descends through ordinary directories so git-ignored descendants can be classified into `ScopeResult.excluded`, while git metadata and submodule boundaries remain outside traversal.
...
- ALWAYS: automatic walking descends through ignored-only ordinary directories so ignored descendants appear in `ScopeResult.excluded` with a git-tracking decision trail ([compliance])
