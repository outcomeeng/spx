...
4. Defer command-line override wiring to the owning domain CLI nodes.
   ...
   - Each `spx validation` and `spx test` command that walks files still needs its owning CLI node to expose those flags.
...
- Consumer commands for `spx validation` and `spx test` still need CLI-level override flags where their owning CLI specs require user-facing `--no-ignore`, `--no-ignore-vcs`, or `--ignore-file <path>` support.
