...
- ALWAYS: every path literal with product meaning consumed inside the file-inclusion subtree — the spec-tree root segment, the gitignore filename, the artifact-directory name set, and the hidden-prefix character — is resolved through a config descriptor; no module reads these values from source text ([review])
- NEVER: a module outside the file-inclusion subtree enumerates an artifact-directory name set, hand-rolls per-tool ignore-flag syntax, or reads a gitignore file — every such responsibility lives inside this subtree and is accessed through its public API ([review])
