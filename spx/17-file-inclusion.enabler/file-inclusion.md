...
SO THAT every command deciding which product paths participate in validate or test work
...
- ALWAYS: every command performing validate or test work that invokes a downstream tool with file-scope arguments forwards a path set consistent with this node's inclusion decision; no such command produces a conflicting included or excluded set for the same request ([audit])
