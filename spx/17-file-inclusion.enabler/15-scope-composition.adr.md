...
- ALWAYS: in the absence of override flags, the git-tracking layer constructs its `LayerState` from a single `git ls-files --cached --others --exclude-standard --full-name -z` invocation against the worktree resolved per `spx/15-worktree-management.pdr.md`; override-modified invocations follow `spx/17-file-inclusion.enabler/21-ignore-source.enabler/21-reader-shape.adr.md` ([audit])
