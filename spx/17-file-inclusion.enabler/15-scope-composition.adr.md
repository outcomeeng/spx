...
- ALWAYS: the git-tracking layer constructs its `LayerState` from a single `git ls-files --cached --others --exclude-standard --full-name` invocation against the worktree resolved per `spx/15-worktree-resolution.pdr.md` ([audit])
