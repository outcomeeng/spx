...
Migration method (apply each batch): rewrite each record decision-first — decision as opening prose (no `## Purpose` / `## Context` / `## Decision` heading), `## Rationale`, optional `## Invariants` (ADR) / `## Product properties` (PDR), and `## Verification`. Map each legacy `[review]` rule to `### Audit` `([audit])`; map a rule already carrying a `[test](path)` link to `### Testing` with the evidence type from its filename. Preserve the decision's concrete contract enumerations verbatim in the opening — env-field and helper lists, signatures, named definitions — because condensing those away reads as content-loss the CI reviewer flags; the legacy `### Recognized by` recognition heuristics are removed with the `## Compliance` block, and the rules they implied map to `### Audit` (kept at the rule's true generality, not narrowed to a recognition path). Update this "Migrated so far" list in the same commit as the migration.
