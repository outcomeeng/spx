...
- NEVER: source adapters emit entries for unregistered node or decision suffixes, including registered descendants below an unregistered ordered node directory ([test](tests/spec-tree-source.mapping.l1.test.ts), [test](../43-entry-recognition.enabler/tests/entry-recognition.mapping.l1.test.ts))
