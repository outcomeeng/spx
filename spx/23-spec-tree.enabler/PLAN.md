...
- **`SPEC_TREE_ROOT` literal in `src/validation/lint-policy.ts`.** A `"spx"` root-directory literal duplicates `SPEC_TREE_CONFIG.ROOT_DIRECTORY`; migrate the consumer to read the owned constant (separate single-source item from the deprecated-suffix migration already landed).
