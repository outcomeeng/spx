...
A name is superseded exactly when an earlier schema version accepted it and the canonical one does not, so distinguishing superseded from invalid requires the prior versions' accepted sets as data. An ordered tuple of complete versions carries that data; a single current schema plus a hand-curated legacy list cannot report which version a name belonged to. Semantic version keys order the tuple and name the version a superseded entry matched: a node-suffix taxonomy change is breaking, so it is a major increment, which keeps the ordering legible without inventing effective dates for schemas whose only fixed fact is their succession. A spec-file form change is breaking on the same footing and increments the major version on its own axis, so the node-suffix axis and the spec-file axis advance independently: an era whose node suffixes are already canonical but whose spec files still carry the bare form occupies its own version between the node-suffix break and the spec-file break. Two independent breaks landing in sequence therefore produce three versions, and a version sharing node suffixes with the canonical version exists exactly because its spec-file form differs.
