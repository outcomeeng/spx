...
- Given an excluded node path in `spx.config.{toml,json,yaml}`, when `spx test passing` runs, then pytest is invoked with `--ignore=spx/{node}/` for that node ([test](tests/python-testing.integration.test.ts))
...
- Config-driven exclusion flag generation: an excluded node path `{segment}` maps to pytest flag `--ignore=spx/{segment}/` ([test](tests/python-testing.unit.test.ts))
