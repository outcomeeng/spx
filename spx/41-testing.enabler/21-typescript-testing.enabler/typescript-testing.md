...
- Given an excluded node path in `spx.config.{toml,json,yaml}`, when `spx test passing` runs, then vitest is invoked with `--exclude=spx/{node}/**` for that node ([test](tests/typescript-testing.integration.test.ts))
...
- Config-driven exclusion flag generation: an excluded node path `{segment}` maps to vitest flag `--exclude=spx/{segment}/**` ([test](tests/typescript-testing.unit.test.ts))
