...
CAN run all spec-tree tests with a single command, honor configured passing-scope exclusions declared in `spx.config.{toml,json,yaml}`, obtain a node's pass/fail outcome through the multi-language registry without naming a runner, read recent status without re-running every test, and inspect failures without streaming passing-test noise into an agent transcript
