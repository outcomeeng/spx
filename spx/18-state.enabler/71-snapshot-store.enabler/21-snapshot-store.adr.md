...
- NEVER: a capture's `(scope, domain)` address shares a run-file directory another node owns — `spx/41-test.enabler/43-last-run-evidence.enabler/11-last-run-file.adr.md` reserves `.spx/worktree/test/runs/` for testing last-run evidence, and the store's enumeration cannot distinguish a foreign run record from a snapshot, so a capture takes a domain distinct from every domain another node reserves ([audit])
