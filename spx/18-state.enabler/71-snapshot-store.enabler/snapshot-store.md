...
SO THAT consumers persisting a whole projected payload addressed by worktree or branch, such as a projection derived from a run's recorded evidence for fast status and last-run reads,
CAN retain, read back, enumerate, and compare successive runs without re-deriving the projection, re-deriving `.spx/` layout, or clobbering another run's snapshot
...
- NEVER: a capture addresses a `(scope, domain)` pair whose run-file directory another node owns — `spx/41-test.enabler/43-last-run-evidence.enabler/11-last-run-file.adr.md` reserves `.spx/worktree/test/runs/` for testing last-run evidence, and enumeration cannot tell a foreign run record from a snapshot, so a capture takes a domain distinct from every domain another node reserves ([audit])
- NEVER: the store imports or binds `spx/15-agent-run-journal.enabler` or `spx/18-state.enabler/71-appendable-journal-store.enabler` — a consumer that derives a snapshot from recorded run evidence reads that evidence itself and hands the store the projected payload, so snapshot persistence stays independent of the event journal ([audit])
