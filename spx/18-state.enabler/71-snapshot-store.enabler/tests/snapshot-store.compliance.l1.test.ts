...
import { STATE_STORE_DOMAIN } from "@/lib/state-store";
...
describe("snapshot store — compliance", () => {
  it("rejects a second snapshot at an already-written address and leaves the persisted document unchanged", async () => {
    ...
    const first = await captureSnapshot(scopeDir, STATE_STORE_DOMAIN.TEST, document, {
      fs,
      now: () => date,
    ...
    const second = await captureSnapshot(scopeDir, STATE_STORE_DOMAIN.TEST, replacementDocument, {
      fs,
      now: () => date,
      randomBytes: () => runBytes,
    });
