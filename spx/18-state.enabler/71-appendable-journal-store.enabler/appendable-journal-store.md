...
- ALWAYS: when event persistence fails after an `append` claims its sequence, the backend releases that claim so retrying the same sequence can succeed ([test](tests/appendable-journal-store-claim-cleanup.compliance.l1.test.ts))
