...
- ALWAYS: when execution stops after the sealing barrier persists, appends remain blocked and a later seal recovers the stale barrier into a complete sealed aggregate ([test](tests/appendable-journal-store-interruption.compliance.l2.test.ts))
- ALWAYS: when append and seal overlap, a successful append is present in the sealed aggregate and hydrated replay, while an append excluded by the sealing barrier rejects with `JOURNAL_ERROR.SEALED` ([test](tests/appendable-journal-store-sealing-race.property.l1.test.ts))
