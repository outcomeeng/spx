...
- Concurrent atomic JSONL publications to one deterministic destination commit exactly one complete generated record and return `STATE_STORE_ERROR.RECORD_ALREADY_EXISTS` for every collision ([test](tests/atomic-jsonl-publication.property.l1.test.ts))
