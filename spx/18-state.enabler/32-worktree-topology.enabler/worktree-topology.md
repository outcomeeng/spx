...
- Given `git worktree list --porcelain` reports a prunable record for a bare-repository pool's repository-named worktree, when `gatherGitFacts` probes, then `GitFacts` excludes that record from observed worktree roots and main-checkout designation returns no path ([test](tests/git-facts.scenario.l1.test.ts))

### Mappings

- In a bare-repository pool the main checkout maps from three signals together — a worktree is the main checkout when its directory basename equals the `origin` remote's repository name, its git-common-dir's parent equals the worktree's parent, and its worktree root appears in the active observed git worktree list; any condition failing maps to not-the-main-checkout, a pool with no agreeing active observed worktree maps to no main checkout, and the mapping is independent of the checked-out branch ([test](tests/main-checkout.mapping.l1.test.ts))
