...
SO THAT the release, spec-domain, session, compact, testing, audit, and review consumers
CAN resolve shared, per-worktree, and tracked product roots, designate the repository main checkout, address `.spx/` branch, worktree, and session scopes, and append and read run records without re-deriving git topology or `.spx/` layout
...
- NEVER: this module imports a consumer domain (release, spec, session, compact, audit, review, testing) — the dependency flows from consumer to state, never the reverse per [`spx/17-state.adr.md`](../17-state.adr.md) ([audit])
