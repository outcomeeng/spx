...
- Deterministic context ingestion — methodology context loading from configured methodology sources plus spec-tree context loading from product root, ancestor specs, decisions, lower-index siblings, tests, and escape hatches without LLM inference
...
- ALWAYS: ingest methodology and spec-tree context deterministically from configured methodology sources, the tracked `spx/` tree, root decisions, ancestor specs, lower-index siblings, co-located evidence links, and node-local escape hatches ([audit])
