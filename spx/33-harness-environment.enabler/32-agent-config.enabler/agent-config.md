...
PROVIDES deterministic reconciliation of repository-local native configuration for every participating coding agent
...
- Given an agent's TOML-serialized native config contains unowned content, when agent config reconciliation writes the managed state, then unowned TOML content is preserved and the managed `spx.harnessEnvironment` state is normalized to the managed table form ([test](tests/agent-config.scenario.l1.test.ts))

### Compliance

- ALWAYS: every participating agent's native settings are modeled as coding-agent outputs under one harness-environment owner ([test](tests/agent-config.compliance.l1.test.ts))
