...
PROVIDES harness-environment agent-config test fixtures — an all-agents-enabled `HarnessEnvironmentConfig` builder, a navigator that reads the managed harness-environment record out of a parsed `.spx` agent-config state, and a record reader that validates a value is a plain object or rejects it
...
- Given the enabled-environment builder, when it runs, then the returned config enables every agent the descriptor registers, over the descriptor defaults ([test](tests/test-harness.scenario.l1.test.ts))
