...
- ALWAYS: the descriptor resolves `methodology`, `instructions`, `agents`, and `pluginBootstrap` defaults through the static config registry ([compliance])
- ALWAYS: explicit methodology `source` and `version` values resolve through the descriptor and malformed methodology fields reject before context-loading consumers run ([compliance])
...
- NEVER: load methodology files or inject methodology context from the descriptor validator; context-loading consumers own those effects ([audit])
