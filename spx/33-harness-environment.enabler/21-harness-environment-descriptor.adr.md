...
- Each agent config carries the session-store root under which that agent writes its session transcripts, so a hook invocation classifies its originating agent by proving the payload transcript path canonically contained by exactly one registered root.
- Compact stdout agent policy selection and `session-start` session identity read one lifecycle fact — the agent the hook payload classifies — so neither is derived from the other and no environment variable participates in either.
...
- ALWAYS: every registered agent config carries the session-store root the hook CLI transport uses to classify a payload transcript path as that agent's ([audit])
- NEVER: compact stdout agent policy selection or session identity resolution reads an environment variable to decide which agent is invoking SPX; both consume the agent the hook payload classifies, per `spx/21-infrastructure.enabler/54-hooks.enabler/21-hook-event-runner.pdr.md` ([audit])
