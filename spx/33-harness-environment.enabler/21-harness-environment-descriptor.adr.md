...
- Compact stdout agent policy selection and `session-start` session identity are separate concerns: `CODEX_THREAD_ID` selects Codex for compact stdout before `CLAUDE_CODE_SESSION_ID`, `CLAUDE_CODE_SESSION_ID` selects Claude Code when the Codex marker is absent, and `CLAUDE_ENV_FILE` selects Claude Code only when both session markers are absent.
...
- NEVER: infer session-start identity from compact stdout agent policy selection or infer compact stdout agent policy from session-start identity; compact stdout policy follows `CODEX_THREAD_ID`, `CLAUDE_CODE_SESSION_ID`, then `CLAUDE_ENV_FILE`, while session-start identity follows its event-specific inputs ([audit])
