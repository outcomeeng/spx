...
- ALWAYS: `spx agent config status` reports installed, missing, stale, incompatible, and failed capability entries deterministically without writes or required network access
- ALWAYS: `spx agent config apply` resolves network inputs and reproduces the exact committed capability versions for each explicitly enabled and available coding agent
- ALWAYS: `spx agent config update` selects only package versions compatible with the declared methodology, persists exact pins through the config owner, and applies those versions
...
- NEVER: capability reconciliation mutates user-scope coding-agent configuration or targets a disabled or unavailable coding agent
