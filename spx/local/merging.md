...
2. Use `/diagnose` to obtain SPX's `worktree-pool` verdict and `mainCheckoutPath`; require `compliant`. Consume SPX's result without reimplementing its Git-layout rules. Check occupancy and cleanliness before the pull; another session's claim or uncommitted work blocks mutation until ownership is resolved.
