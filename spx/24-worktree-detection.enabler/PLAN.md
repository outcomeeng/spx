...
(Consumer 2 — the precommit dist-rebuild gate — was drafted on this branch but taken back
out: routing the hook through the tsx-based classifier has an unresolved ordering tension
and the gate is unspecified, so it ships in its own governance PR. See Remaining step 1.)

## Remaining (separable from this branch's consumer wiring)

1. **Precommit dist-rebuild gate + governance.** `lefthook.yml` keeps the original bash
   heuristic (`dirname(common-dir) == toplevel`), which misfires in a bare pool. The
   corrected gate over the tested `isMainCheckout` is drafted and **preserved on the
   `wip/precommit-main-checkout-gate` branch** (commits `94e6c25`, `fbe4239`, `dac762e`,
   `f17dc10` — the `src/lib/precommit/main-checkout-gate.ts` module + the lefthook wiring).
   Two things to settle in the governance PR before re-landing it:
   - **Ordering tension**: the gate runs TypeScript via `tsx` (a devDep), so install-before-gate
     runs install in feature worktrees while gate-before-install cannot run without `tsx`.
     Resolve by a tsx-free gate (POSIX-sh re-derivation, accepting divergence from the tested
     classifier) or by accepting one trade-off — decide with the governing decision.
   - **Governance**: author the dist-rebuild-on-pull decision under `spx/43-precommit.enabler`
     citing `spx/15-worktree-management.pdr.md`, closing the open item in
     `spx/43-precommit.enabler/ISSUES.md` (PR #84).
2. **`gatherGitFacts` `--git-common-dir`-failure fallback has no injection test.** Removing
   `isRootWorktree` removed its injection test of the analogous fallback;
   `gatherGitFacts` carries the same fallback (read fails → non-bare single-tree shape) with
   no test. Add an injection test under `spx/24-worktree-detection.enabler/tests/` (mock
   `--show-toplevel` success + `--git-common-dir` exit 128; assert `{ commonDir: join(root,
   ".git"), commonDirIsBare: false }` and `isMainCheckout` true) plus the governing spec
   assertion. (Defensive branch — unreachable in real git, hence deferred.)
3. **Reconcile `spx/36-session.enabler/26-worktree-detection.adr.md`**: the two general
...
4. **Co-locate the resolver tests** (`tests/root-resolution.scenario.l1.test.ts`,
...
5. **Fix stale `spx/PLAN.md` operating-constraint**: its "Operating constraints" section
