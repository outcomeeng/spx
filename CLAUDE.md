...
- ⚠️ **`[audit]` vs `[test]` is the verification MECHANISM, not a lifecycle marker.** `[test]` is the default for testable assertions (scenario/mapping/property/automatable compliance); `[audit]` (legacy spelling `[review]`) is for judgment constraints no automated test can verify; `[eval]` is for LLM-driven behavior with a structurally scoreable verdict. A testable assertion is `[test]` (its co-located test is written via `/applying`), never an `[audit]` "placeholder."
