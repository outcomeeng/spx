...
- 🛑 **STOP TRIGGER — running tests. Every agent-run `spx test` command MUST include `--agent`; NEVER tail logs, probe PIDs, poll, or wrap the command in a retry or wait loop.** `--agent` captures raw runner output and prints one compact final summary. Run each command once in the foreground. The product has ONE test verb, `spx test`; choose by situation:
  - **Verify work in progress** → `spx test --agent --changed [--base origin/main]` — focused by the branch/worktree diff, or `tsx src/cli.ts test --agent --changed [--base origin/main]` when changing `spx test` itself. To force one node or file, use `spx test --agent spx/<node>` or `spx test --agent spx/<node>/tests/<file>`.
  ...
  - **Deliberate full local run (rare)** → `pnpm run build`, then `spx test --agent`, with the specific written justification required by `spx/local/merging.md`.
  - **Coverage / watch** → `pnpm run test:coverage` / `pnpm run test:watch` are human-interactive paths. Agents never invoke raw Vitest or `pnpm test`.
...
Access it read-only only to inspect plugin behavior and form improvement suggestions. The writes it admits are a Change draft in that repository's untracked `.spx/` change store and the verification-run journal of that draft's Change audit, and nothing else.
Never edit, branch, commit, test, audit, PR, or merge its product content there; a separate session in that repository has its own instructions.
...
| Run tests for work in progress                                            | `spx test --agent --changed [--base origin/main]` — diff-selected touched scope only (use `tsx src/cli.ts test --agent --changed [--base origin/main]` when changing `spx test` itself); use `spx test --agent spx/<node>` only to force one known node. |
