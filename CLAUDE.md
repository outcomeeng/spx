...
- ⚠️ **`[review]` vs `[test]` is the verification MECHANISM, not a lifecycle marker.** `[test]` is the default for testable assertions (scenario/mapping/property/automatable compliance); `[review]` is only for judgment constraints no automated test can verify. A testable assertion is `[test]` (its co-located test is written via `/applying`), never a `[review]` "placeholder."
