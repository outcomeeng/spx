...
- ⚠️ **NEVER discard work with `git checkout -- <path>`, `git restore`, `git stash`, `git reset --hard`, `git clean -f`, or plain `git push --force`** — the first five wipe uncommitted local changes; `git push --force` unconditionally overwrites a remote ref a concurrent agent may have advanced. Hand them off to the user; if you need to discard changes, ask the user to do it.
