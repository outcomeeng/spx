...
- ⚠️ **NEVER discard work with `git checkout -- <path>`, `git restore`, `git stash`, `git reset --hard`, `git clean -f`, or plain `git push --force`** — these wipe uncommitted changes or unconditionally overwrite a remote a concurrent agent may have advanced. Hand them off to the user; if you need to discard changes, ask the user to do it.
